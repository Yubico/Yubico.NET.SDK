...
**Current acceptance (2026-09-24): 18/72 checked, 54 pending.** The checklist
...
ISC-4/37/53/56/62 remain open. Fresh smart-card hardware is blocked before
open by sharing contention; Windows/Linux native and hardware routes remain
deferred. See [current evidence](#verification)
...
listener have the bounded evidence indexed in [status](docs/plans/yubikit-async-boundaries/00-status.md).
Smart-card continuation/secure-failure refusal is committed at `cc13b518`;
the expanded operation registry and selected smart-card verifier at `29ec80f6`.
Earlier device results are not automatically evidence for these source changes.
The listener result is accepted for ISC-33 and bounded protected-chain proof
for ISC-19; checked IDs are enumerated in [Criteria](#criteria).
ISC-4/37/53/54/56/62 remain open at full
scope. Windows/Linux native and other hardware/operator work is deferred,
not passed. Current `.8` measurements are snapshots, not final-source
performance proof.

**ISC-33 decision:** The orchestrator accepted the real matching, selected-key removal
and late-drain evidence at the bounded grade in [Verification](#verification).
Manager Close returned a dead-port error, not successful per-device close.
Permission and normal-user Input Monitoring experience on other hosts remain
separate follow-up work; the no-open removal experiment is inconclusive and
not a current gate. No repeat unplug or Stop probe is needed for this checkpoint.
...
**Current implementation:** worktree `/Users/Dennis.Dyall/Code/y/worktrees/yubikit-async-boundaries`, branch `yubikit-async-boundaries`. Listener-generation cleanup is committed at `69946394`, direct feature-report ownership at `db378ffc`, and direct report input ownership at `4f361504`; OTP transport-fault recovery and public lifetime documentation at `efde3ef0`, and migrated-operation profiles and portable boundary controls at `add0dc73`. Native source `71a23cd0269c968c1d9420eddb2e2fec71e0cc29` produced private-published `.8` via workflow 35955737172. Native commits `541cfb09` and `760d0416` add verification and contract documentation only; they did not produce `.8`. The earlier `.7`, pre-slice baseline `a7f2cae8`, and demo baseline `99e082c4` remain historical.

**Earlier host troubleshooting (historical):** Selected-key PC/SC async
transaction/read/reopen passed after an earlier sharing violation. OTP opening
initially returned `0xE00002E2` on three interfaces; Input Monitoring was a
possible, unconfirmed explanation. After replug, an OTP discovery identity
read exceeded its two-second budget and correctly retained an unrecovered claim.
In a later isolated process the late read released the claim about 1.3 seconds
later; three read-only feature GET/open/dispose cycles then passed on serial
20260533, and serial 31683481 passed three without delayed release. A controlled
16-test OTP suite verifies refusal until late native completion. These old
sharing/access failures are not current blockers or proof of permanently hung
driver recovery. Current `.8` typed and direct OTP evidence is below.
...
**Current workflow:** [Status](docs/plans/yubikit-async-boundaries/00-status.md) is the finite active dispatch list; the [program design](docs/plans/yubikit-async-boundaries/03-program-design.md) records the slice finish rules. The orchestrator owns acceptance/classification and Engineers own disjoint implementation and probes. D30 supersedes D14/D25's earlier micro-walkthrough requirement for coherent in-scope internal work; escalate material scope changes, destructive operations and unresolvable blockers. The [earlier architecture](docs/plans/yubikit-async-boundaries/addenda/earlier-multi-key-architecture.md), [earlier program design](docs/plans/yubikit-async-boundaries/addenda/earlier-multi-key-program-design.md) and [multi-key addendum](docs/plans/yubikit-async-boundaries/addenda/multi-key-capacity.md) retain deferred research, not the active checklist.

## Problem

The SDK exposes asynchronous device operations, but a `Task`-returning signature does not prove that the initiating thread remains responsive. At the S0 baseline HID wrappers executed synchronous native reports before returning completed tasks, and PC/SC used per-call thread-pool offloading with a synchronous transaction-begin wait. These specific routes have since migrated; initialization, callbacks, cancellation, discovery and teardown still require distinct lifetime classification.
...
The remaining engineering gap is **complete** traceability. A Core-only inventory and partial operation registry now exist, but outstanding sites and missing direct/other-platform rows do not establish universal ownership, lifetime, tests or runtime evidence. Managed seams and Native AOT link success do not prove unexecuted native completion or hardware behavior.

### Baseline observations at S0 (historical, not current implementation)

| Area | Baseline source evidence | Planning observation at S0 |
|---|---|---|
...
- [x] ISC-19: An admitted SCP-protected chained exchange preserves valid MAC/chaining state for the next exchange when reuse is reported safe.
...
- [x] ISC-33: macOS teardown reaches native quiescence acknowledgment before callback context/device storage is reclaimed.
...
| Entry route | Applet/convenience path, typed raw connection path, retained synchronous compatibility/lifecycle path |
...
The ISC-33 row below is its acceptance decision. Earlier 1,457/1,460/1,465
Core results and 83 resilience counts predate the current Core 1,492 passed/
3 skipped and resilience-fast 88 checkpoint. The finite current evidence index
is [status](docs/plans/yubikit-async-boundaries/00-status.md).

Owner: orchestrator for acceptance; Native/Route Engineers for source and probes. The earlier feature-report checkpoint was `db378ffc`; listener-generation cleanup was subsequently committed at `69946394`. `.8` native producer source is `71a23cd0269c968c1d9420eddb2e2fec71e0cc29`. Later unpushed native commits `541cfb09` (export checker) and `760d0416` (persistent-registration contract comment and test) do not change the `.8` runtime/package and are not producer revisions. No full cross-platform acceptance is claimed. The later `.8` selected-key listener-removal callback and late-drain evidence supports ISC-33 at the stated macOS teardown grade below; the earlier `.8` FIDO input-owner physical-unplug claim remains `.6`-only. [Status](docs/plans/yubikit-async-boundaries/00-status.md) has the newer bounded measurement; the older `.7` table below is historical.
...
| ISC-33 | On pinned `.8`, the macOS Native AOT listener probe with real vendor-filtered `IOHIDManagerOpen` result 0 observed a matching callback on an already attached topology (entry 4294970786, not attributed to the selected key). On operator removal of selected serial 31683481, entry 4305321102 delivered a matching `Removed` hint. A verification-only decorator held callback return after production delivery: Stop request 1 < callback exit 2 < native manager Close return 3 < explicit unschedule 4 < manager release 5 < Stop return 6; unschedule/release did not run while the callback was held. Close returned `0x10000003` (`MACH_SEND_INVALID_DEST`) for a physically dead per-device port, **not** successful per-device close. Apple's [IOHIDManager.c](https://github.com/apple-oss-distributions/IOKitUser/blob/main/hid.subproj/IOHIDManager.c) shows Close marks the manager closed and unschedules its run loop before returning a per-device close error. A separate real matching-callback late-drain probe (open result 0, chosen Stop timeout 150 ms) observed the original generation retained through timeout, restart refusal, callback exit, cleanup and subsequent successful restart; it held a matching callback, not a removal callback. Eleven controlled listener tests, full Core 1,465 passed/3 skipped, resilience-fast 88 passed and complexity nine changed shipping methods passed. Independent cross-vendor review: PASS WITH NOTES. | **Accepted for macOS listener callback-context/device-storage teardown at this observed native/hardware grade.** Run-loop exit and callback drain precede release; manager `isOpen` becomes false and its run loop is unscheduled even though per-device close reported an error. This is not proof for every macOS version, all teardown races or a permissionless host. Vendor `0x1050` filtering excludes unrelated keyboards, but manager open can still trigger Input Monitoring permission for YubiKey OTP keyboard interfaces. Orchestrator tracks permission/user-experience risk as a follow-up, not an ISC-33 teardown blocker. The no-open removal attempt exceeded its 180-second probe by at least ten minutes without timely callback: inconclusive, not a runnable current gate or proof open is necessary. No further physical action is required for this acceptance. |
| ISC-31 | `MacOSFidoHidConnection` retains the persistent native input owner for typed FIDO; the public direct report connection `MacOSHidIOReportConnection` now delegates to that owner instead of scheduling `CFRunLoopRunInMode` on the caller. Pending typed reads detach their expected reader under lock on cancellation, without cancelling native input or publishing a terminal result; late reports queue, and a stale cancellation token cannot detach a replacement reader. Five read-cancellation, eight IO-compatibility and three facade tests pass; six old legacy tests were removed after moving meaningful coverage. On serial 31683481 the `.8` native-AOT `--expert-io` probe observed a 6,011 ms timeout, then same-connection INIT/getInfo and dispose/reopen/getInfo passed without operator interaction. Independent cross-vendor review: PASS. | macOS HID **input waits** use the event-delivery owner; macOS OTP feature reports are feature GET/SET, not input callbacks. Public synchronous open/get/set/dispose remain explicitly blocking. The separate ISC-33 listener callback evidence is now accepted at its bounded grade; ISC-53 still covers other synchronous public waits. The expanded registry includes direct raw rows but does not establish universal ISC-4 or Windows/Linux hardware execution. |
| ISC-32 | Typed macOS FIDO output and public direct IO SET use the connection-owned FIDO worker; typed OTP feature GET/SET and the public direct report connection `MacOSHidFeatureReportConnection` use the connection-owned OTP worker, not native callback report APIs. The feature facade now opens/reads descriptor metadata on that worker and forwards GET/SET and checked shutdown through it. It preserves synchronous compatibility-path caller blocking; one owned worker admits one operation at a time, without a process-global capacity bound. Direct feature GET owns an eight-byte array: short native replies (0–8) stay zero-padded, lengths greater than eight fault and zero the buffer. Typed OTP send remains exactly eight bytes; direct feature SET admits arbitrary length for native validation. Accepted native calls drain before checked close. Eleven feature-compatibility and nine IO-compatibility tests passed after the whole-array ownership fix (both direct report GetReport facades return the owned array through `MemoryMarshal.TryGetArray` without a second uncleared copy; identity is tested). Independent review: PASS WITH NOTES. Native-AOT host built from this worktree using pinned `.8` passed `--expert-feature --serial 31683481` (3/3 eight-byte feature GETs plus read-only Management info via feature SET/GET and dispose/reopen, serial matched) and separately `--otp-info` (3/3 typed queries) on the same key. | Explicit connection-owned worker fallback for all identified macOS output/feature directions, not a claim that callback GET/SET APIs are asynchronous. Public direct report open/Get/Set/dispose intentionally block; capacity is local to the connection, not proven process-global. ISC-33 still includes listener/manager callback quiescence; ISC-53 covers remaining public synchronous waits. No physical touch/unplug or other-platform native execution. |

**Earlier listener source checkpoint (before ISC-33 acceptance):** `MacOSHidDeviceListener` roots each manager/run-loop/callback generation through listener-thread cleanup. Stop waits outside the lock; concurrent Stop/Dispose share a deadline, and timeout retains the generation and refuses restart until cleanup. Six controlled lifetime tests and four no-change on-host integration tests passed, but the latter observed no real arrival/removal callback. The later selected-removal and late-drain evidence is in the ISC-33 row above; this earlier checkpoint alone did not establish native callback quiescence.

The following evidence-collection paragraph is a **historical pre-acceptance
snapshot**; its statement about ISC-33 being unchecked describes that earlier
review moment only. The orchestrator subsequently accepted ISC-33 in the row
above, and no current checkmark is pending for this teardown route.

**Mac evidence-closure collection (no additional checkmark yet):** On checkout `90443131`, the macOS arm64 Native AOT `MacOSHidRouteVerification --listener-drain` executable using pinned `.8` observed an actual IOHIDManager matching callback on the already attached topology: HID entry ID 4294970786 on the final verification run, not asserted to belong to the selected YubiKey. A verification-only decorator over the real native listener held the callback before forwarding it to the production listener, and recorded exit before real manager close, unschedule, release and Stop return. No production listener hooks remain. On a subsequent operator-assisted selected-key physical removal (serial 31683481, HID entry 4305321102), actual manager open returned 0 and a real removal callback delivered a matching `Removed` hint. Holding callback return after production delivery blocked cleanup; stop request 1 < callback exit 2 < manager close return 3 < unschedule return 4 < manager release return 5 < Stop return 6. IOHIDManagerClose returned `0x10000003` (`MACH_SEND_INVALID_DEST`) on the physically dead port, not a successful per-device close. Apple `IOHIDManager.c` source inspection shows manager close unschedules/marks closed before returning a per-device close error; the listener logged the error and continued post-run-loop cleanup. A real matching-callback late-drain probe (open result 0) passed Stop timeout retention, restart refusal and cleanup after callback exit before restart. Independent cross-vendor review: PASS WITH NOTES after the probe/source correction. An operator `--listener-remove-without-open` attempt produced no callback over about ten minutes, exceeding its 180-second wait: BLOCKED/inconclusive, **not** evidence that opening is necessary. Vendor-only matching (`0x1050`) excludes unrelated keyboards, but YubiKey OTP keyboard interfaces still pose an Input Monitoring permission risk on other macOS hosts; this is not permissionless-host proof. The orchestrator keeps ISC-33 unchecked pending its acceptance judgment, not because removal/late drain were unobserved; ISC-53/56/62 also remain open. These selected-host observations do not establish every teardown race or close-error disposition.

**Earlier matching-probe verification before removal acceptance:** Core 1,460 passed/3 skipped; BoundaryInventory 32 passed; listener lifetime six passed; resilience-fast 83 passed. The later accepted ISC-33 row records the subsequent Core 1,465/3 skipped and resilience-fast 88 results.

ISC-53: [retained synchronous paths](docs/architecture/raw-access-tiers.md#retained-synchronous-compatibility-paths) now maps direct reports, typed fallbacks, transaction begin/end, applet OTP initialization, forwarding, monitor and manager shutdown to public entry and existing tests or explicit gaps. Scanner sites are documented/outstanding rather than a zero-gap public reachability proof; Windows/Linux and external implementations remain open. ISC-56: the `BoundaryInventory` map adds smart-card protocol and discovery logging sites to existing PC/SC/OTP connection and protocol coverage, plus a real logger command/response sentinel negative control for `PcscProtocol`. External exception messages and other applet/migration logs have not been exhaustively tested. Both universal criteria stay unchecked.
...
**Current source classification and test-link coverage (source `fdedd61c` plus uncommitted expansion):** `BoundaryInventory` classifies **205 outstanding Core sites**: 134 native imports, 3 native exports, 24 blocking waits, 15 scheduling sites, 21 pre-task-return dispatch gaps, 6 callback registrations, 0 delegate conversions and 2 unmanaged callback addresses. Classified is not verified. The independent registry requires **23 operation rows and 45 named Fact/Theory profiles**: typed macOS FIDO 4, typed macOS OTP 4, portable PC/SC 5, synchronous macOS direct input 4, direct feature 4, listener 2. The link gate resolves owners, symbols, roles and runnable named methods; it does not recursively execute profiles, prove synchronous waits drain on every path or establish universal ISC-4. Windows/Linux, external implementations and further public reachability remain open. **Historical pre-expansion checkpoint:** the 13-row/26-profile registry omitted direct/listener rows, and an earlier Core manifest had 198 outstanding sites (130 native imports, 24 waits, 15 scheduling, 21 dispatch gaps, 6 callback registrations, 0 delegate conversions, 2 addresses). An earlier feature slice recorded 200 sites. Those are dated source snapshots, not current counts. The earlier targeted `BoundaryInventory` run passed **28** tests (13 scanner, three registry, six responsiveness, six diagnostics); the matching-probe checkpoint subsequently passed 32. Neither run is asserted for the uncommitted expansion. Diagnostics inventory logging in three files and exercise real PC/SC and OTP sentinel paths, including full payloads and leading five-byte prefixes (not arbitrary fragments); externally supplied exception messages and other migration logs are not comprehensively covered, so ISC-56 stays open.

**ISC-19 accepted at bounded managed grade; uncommitted PC/SC source:** Plain command/response continuation failure marks the base protocol recovery-required. After protected state advances, first transport failure, invalid response MAC, interrupted protected continuation and intermediate short-response rejection latch that same protocol and preserve the original exception without replay. Wrapped plain SELECT continuation uses one shared notification (no duplicate secure callback). A valid authenticated terminal application error remains reusable. An independently modeled command-MAC card verifies that cancellation *during* one protected response continuation completes the chain and permits the next protected command with valid MAC; this is one controlled profile, not physical cancellation or all card configurations. Three plain continuation cases and the plain SELECT case were observed red then green; some protected tests first failed because of their fixture and do **not** provide meaningful red evidence. Focused results: secure filter 159 passed/2 skipped (including 18 SCP cases); latest full Core 1,492 passed/3 skipped. A new protocol over the same borrowed raw connection can bypass the per-protocol latch: raw callers must reopen rather than infer safe reuse. ISC-20 stays open (universal no-replay scope); neither this result nor the five portable PC/SC registry rows checks ISC-37, whose monitor/discovery and custom synchronous-fallback lifecycle map remains incomplete.

**Current software verification at this working-tree checkpoint:** Core 1,492 passed/3 skipped; secure filter 159 passed/2 skipped; PublicApi 22, Management 86, Piv 209, Fido2 471, YubiOtp 180 and resilience-fast 88 passed. The latter module runs preceded the two final receiver-logic changes; the subsequent full Core run covered those changes, not those other module suites. Complexity checked 18 changed shipping methods within cyclomatic 10/cognitive 20; `ScpTransmit` measured 10/9 after removal of redundant null branches, without another framework. Independent software review: PASS. Parent owns later broader suite/hardware reconciliation; these are reported runs, not reruns by this documentation edit.

**Latest selected-key hardware outcome (distinct from earlier passing smart-card runs):** The `.8` Native AOT host passed five FIDO scenarios and three typed OTP info cycles on selected serial 31683481, firmware 5.7.4. Discovery observed **two** attached keys (25555459 and 31683481); only 31683481 was selected for operations, so no single-attached-key claim follows. The broad Core smart-card hardware integration test failed; the isolated `--smartcard` child for serial 31683481 also returned `SCARD_E_SHARING_VIOLATION` **before open**. No transaction or read completed on this latest source. The new read-only smart-card scenario provides phase diagnostics under a 20-second watchdog; it does not prove an SDK API defect or clear the current hardware cell. Earlier selected-key smart-card success remains historical/version-specific. The operator is away: no replug, presence request, configuration change or retry is requested.
...
**Historical post-listener checkpoint, not the latest run:** Core 1,457 passed/3 skipped; PublicApi 22 and resilience-fast 83 passed. Targeted feature and IO compatibility runs passed 11 and nine; complexity passed 33 then-changed shipping methods. The later full Core 1,492/3 skipped and 18-method complexity check above supersede these as current software results. The `.6` input-owner unplug and `.8` listener-removal probes are separate version-specific routes. Windows/Linux native and hardware checks remain deferred.
...
`IHidInterface`/`IFindHidInterfaces`, both FIDO/OTP wrappers and six platform implementations;
...
| Windows reports: `Native/Windows/HidD/WindowsHidReportAccess.cs:65-109,155-183,214-222` | FIDO uses read/write access and synchronous `ReadFile`/`WriteFile`; OTP uses zero desired access and feature reports. | Open, report, removal and release each need disposition; no owned overlapped completion path currently exists. |
...
|---|---|---|
| `src/Core/src/Native/Windows/HidD/IWindowsHidReportAccess.cs:17-31` and `WindowsHidConnectionOpenFailureTests` | Synchronous open/report/dispose interface; current tests cover constructor-failure cleanup. | Owned asynchronous request state, delayed completion, cancellation races or completion-before-free. |
...
| Platform/discovery/report machinery | Candidate internalization families: `IHidConnection`/`IHidInterface`, their finders/descriptors/listeners, and smart-card reader/find/factory/listener plumbing. Review platform helpers and protocol processors for the same consumer-use test. Exact member closure remains to be checked before changes. | `src/Core/src/PublicAPI.Unshipped.txt:582-643,666-710` exposes these families. `FidoHidConnection` and `OtpHidConnection` are already internal wrappers, so lower-level `IHidConnection` visibility is distinct from public `IFidoHidConnection`/`IOtpHidConnection`. `SmartCardConnectionFactory.CreateAsync(IPcscDevice, ...)` and `IHidInterface` report-opening signatures mean related types must move together. |
...
| ISC-62 | Historical [`.3` comparison datasets](docs/plans/yubikit-async-boundaries/00-status.md#measured-comparison-not-a-performance-acceptance-claim) and the reproducibility commands below give 10 completed fresh-child lifecycle samples per side on the same host/key and `.3` package, BEFORE source `65964966`, AFTER `89420aa6`. Invocation medians 47.881 → 28.538 ms, task terminal 48.467 → 47.953 ms, lifecycle 78.456 → 80.144 ms; AFTER max lifecycle ~151 vs ~100 ms. These medians are not a performance pass: they are not comparable to current `.8`, and lack an approved budget, native durations, allocation or idle activity. No-input samples did not complete both lifecycle phases; the earlier BEFORE block had 8/10 completed, one failed and one unattempted, and is not counted in the paired dataset. Descriptive only; ISC-62 remains open. |
