...
**Status (2026-09-24):** Pinned private-published `1.18.1-async.8` is source-bound to native `71a23cd0`; packaged consumers and fresh private-feed restores passed. Managed macOS expert input and feature facades now share the typed FIDO persistent input owner and OTP owned feature worker respectively. Independent review accepts ISC-32 for classified macOS output/feature directions. **ISC-5, ISC-31, ISC-32, ISC-38, ISC-39, ISC-41, ISC-43–49, ISC-51, ISC-52 and ISC-60 are checked (16/72; 56 pending).** ISC-33 (including discovery-manager callback quiescence), ISC-53 (all public synchronous waits), ISC-56 (all migration diagnostics) and universal ISC-4 remain open. `.8` selected-key expert feature and typed OTP info passed separately, not new touch or unplug. Windows/Linux native/hardware routes remain deferred pending user direction. The earlier `.8` current-profile dataset predates both expert facades and lacks native-only duration and pending count; ISC-62 remains open. [Current status](docs/plans/yubikit-async-boundaries/00-status.md) records grades and limits; dated evidence below remains historical.
...
**Last full Core run:** 1,451 passed/3 skipped after the expert-feature fix. PublicApi 22, YubiOtp 180 and resilience-fast 77 passed; Fido2 471 and the 35 focused OTP protocol tests passed at earlier checkpoints. After the whole-array fix, the targeted feature and IO compatibility runs passed 11 and nine respectively. `dotnet toolchain.cs complexity` passed 21 changed shipping methods at cyclomatic ≤10/cognitive ≤20; manually split verification/harness/test methods were excluded and are not tool-certified. The prior targeted 28-test inventory run is recorded above, not presented as a post-fix rerun. The earlier `.8` selected-key native-AOT five normal, active-cancel and three read-only OTP info scenarios preceded the facades; current expert-feature and typed OTP info evidence is in ISC-32 above. No new touch or unplug at `.8`; physical removal remains `.6`-only. Windows/Linux native runtime and hardware tests are deferred pending user direction; packaged consumer jobs do not prove them.
