...
**Current acceptance (2026-09-24): 17/72 checked, 55 pending.** ISC-33 is
accepted for macOS listener callback-context/device-storage teardown on the
observed vendor-filtered manager path; see the ISC-33 verification row below.
ISC-53/56/62 and universal ISC-4 remain open. The two evidence snapshots
immediately below predate this decision; their 16/72 totals and “not yet checked”
statements are historical, not the current status.

**Status (2026-09-24):** Pinned private-published `1.18.1-async.8` is source-bound to native `71a23cd0`; packaged consumers and fresh private-feed restores passed. The committed direct IO/feature report facades share typed connection owners. The macOS topology-listener slice, committed at `69946394`, retains each manager/callback generation until its run-loop thread finishes and native cleanup succeeds; independent review passed after the concurrent-stop fix. **ISC-5, ISC-31, ISC-32, ISC-38, ISC-39, ISC-41, ISC-43–49, ISC-51, ISC-52 and ISC-60 remain checked (16/72; 56 pending).** Six controlled listener-lifetime tests and four earlier on-host listener integration tests did not observe actual callback drain. Later real-native probes observed matching-callback drain, selected-key physical removal callback and matching `Removed` hint, and late matching-callback timeout/restart gating before native cleanup. On selected removal, actual manager open returned 0; after callback exit, manager close returned nonzero `0x10000003` on the dead port, then unschedule, release and Stop returned in order. This is callback-context teardown evidence, **not** a successful per-device close or permissionless macOS-host claim. Independent cross-vendor review: PASS WITH NOTES. An operator `--listener-remove-without-open` attempt with no callback after about ten minutes (longer than its 180-second probe wait) is BLOCKED/inconclusive, not proof that opening is necessary. The orchestrator has not yet checked ISC-33; its acceptance judgment remains separate from the Engineer's probe result. ISC-53 (all public synchronous waits), ISC-56 (all migration diagnostics) and universal ISC-4 also remain open. Windows/Linux native/hardware routes remain deferred pending user direction. A new `.8` current-profile snapshot from checkout `90443131` plus a recorded shipping diff follows the older pre-facade profile; neither is final-source performance proof, and native-only duration and pending count remain unavailable, so ISC-62 stays open. [Current status](docs/plans/yubikit-async-boundaries/00-status.md) records grades and limits; dated evidence below remains historical.

**ISC-33 decision (reviewer PASS WITH NOTES; orchestrator accepted):** Real vendor-filtered manager matching, selected-key removal and late-drain observations support the macOS teardown criterion at the bounded grade in [Verification](#verification). A dead-port per-device Close error is not a successful device close; the manager was closed and unscheduled before explicit unschedule/release, after callback exit. Permissionless operation and normal-user Input Monitoring experience on other hosts are **not** established. The orchestrator owns this permission follow-up separately; ISC-53/56/62 and universal ISC-4 remain open. The no-open removal experiment is inconclusive, not a current runnable gate. No further operator unplug or Stop probe is required for this session.
...
All criteria began unchecked; ISC-5, ISC-31, ISC-32, ISC-33, ISC-38, ISC-39, ISC-41, ISC-43–49, ISC-51, ISC-52 and ISC-60 are now verified below (17/72; 55 pending). **Production milestone P is ISC-1 through ISC-64; exploration milestone X is ISC-65 through ISC-72.** P may ship without X, but must never be reported as all 72 criteria complete. Universal criteria are parameterized by the explicit operation/adapter registry; a missing row fails verification. Slices may contribute evidence to the same criterion, but no early slice claims universal acceptance.
...
**Current scope verification after production hooks were reverted:** full Core 1,460 passed/3 skipped; BoundaryInventory 32 passed; listener lifetime six passed; resilience-fast 83 passed. These are the final bounded-scope counts, distinct from the historical runs below.
...
Other current bounded evidence: the Core-only registry has 13 required migrated-route operations with 26 valid named runnable Fact/Theory profile links; public direct raw IO and feature routes are not yet added operation rows. The targeted `BoundaryInventory` run passed **28** tests at the prior checkpoint: 13 scanner, three registry, six responsiveness, six diagnostics. Diagnostics inventory logging in three files and exercise real PC/SC and OTP sentinel paths, including full payloads and leading five-byte prefixes (not arbitrary fragments); externally supplied exception messages and other migration logs are not comprehensively covered, so ISC-56 stays open. The registry covers macOS typed FIDO/OTP and portable PC/SC, not universal ISC-4. The current Core inventory assertion lists **198 outstanding sites**: 130 native imports, 24 waits, 15 scheduling, 21 pre-task-return dispatch gaps, six callback registrations, zero delegate conversions and two unmanaged callback addresses; these are documented, not cleared. The 200-site breakdown belongs to the earlier feature slice.
