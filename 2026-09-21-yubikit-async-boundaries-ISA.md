...
Collected in the working tree before the implementation checkpoint commits, the pinned `.8` Native AOT `--probe` on selected
serial 31683481 passed five production FIDO scenarios and three typed OTP info
cycles without touch or replug; this is selected-device evidence, not a new
smart-card result or cross-platform closure.
...
performance proof. Current implementation checkpoints are `8d13fd1e` (logging
and FIDO response refactor) and `8ebcd026` (synchronous drain, scan, registry
and docs); neither is a new native package or acceptance checkmark. The selected-
device probe was collected before those commits against the working-tree source,
not rerun at either commit; no shipping behavior changed in the test-cleanup fix.
...
and 1,492 Core results and 83 resilience counts predate the reported Core
1,508 passed/3 skipped and resilience-fast 88 checkpoint. That full Core run
predates the SELECT hex-log restoration; later focused tests are recorded below.
The finite current evidence index
...
**ISC-19 accepted at bounded managed grade; PC/SC recovery committed at `cc13b518`:** Plain command/response continuation failure marks the base protocol recovery-required. After protected state advances, first transport failure, invalid response MAC, interrupted protected continuation and intermediate short-response rejection latch that same protocol and preserve the original exception without replay. Wrapped plain SELECT continuation uses one shared notification (no duplicate secure callback). A valid authenticated terminal application error remains reusable. An independently modeled command-MAC card verifies that cancellation *during* one protected response continuation completes the chain and permits the next protected command with valid MAC; this is one controlled profile, not physical cancellation or all card configurations. Three plain continuation cases and the plain SELECT case were observed red then green; some protected tests first failed because of their fixture and do **not** provide meaningful red evidence. At that checkpoint, secure filter 159 passed/2 skipped (including 18 SCP cases) and full Core 1,492 passed/3 skipped; the newer software checkpoint is below. A new protocol over the same borrowed raw connection can bypass the per-protocol latch: raw callers must reopen rather than infer safe reuse. ISC-20 stays open (universal no-replay scope); neither this result nor the registered portable PC/SC rows checks ISC-37, whose monitor/discovery and custom synchronous-fallback lifecycle map remains incomplete.

**Latest reported software verification, not rerun by this documentation edit:** At the source now recorded in `8d13fd1e` and `8ebcd026`, with SELECT application-ID hex restored and the HID discovery seam, full Core **1,511 passed/3 skipped**, PublicApi 22, Fido2 471, YubiOtp 180 and resilience-fast 88 passed before those commits; no post-commit rerun is inferred. Focused suites: adapter registry 17, public-sync registry five, PC/SC lifetime 30 and HID scan boundary three passed; after the held-scan test cleanup fix, the targeted three passed again. `FidoReceiveResponse` was separated into terminal/validation/copy helpers without changing control/error precedence or buffer zeroing; all 12 changed shipping methods were within cyclomatic 10/cognitive 20. Complexity checks do not cover untracked tests or prove whole-project complexity. Docs QA and architecture validation passed at the recorded source. Independent review: PASS WITH NOTES after one bounded fix for a test false negative; cleanup now releases the held discovery delegate, awaits the actual scan and only then disposes gates. No production defect was established by that note. Public SELECT application-ID hex logging remains. Earlier Core 1,508/3 and 1,492/3, secure filter 159/2 and other module counts above are historical source snapshots, not this checkpoint. The selected-key `.8` FIDO/OTP probe below adds bounded working-tree hardware evidence; no fresh PC/SC test was run, and pre-open smart-card sharing contention remains unresolved.

**Latest selected-key hardware outcome (distinct from earlier passing smart-card runs):** Collected from the working tree before `8d13fd1e`/`8ebcd026`, the published Native AOT host with pinned `.8` ran `--probe` and `--otp-info` on selected serial 31683481, discovered **one key** (PID 0407, fallback identity), and reported firmware 5.7.4. The implementation source is now recorded in those commits; neither command was rerun after commit. All five production FIDO scenarios passed, including pending-read disposal/reopen and pre-dispatch cancellation; typed OTP info passed three cycles. No touch or replug was performed. First initialization took 229 ms and a subsequent one 20 ms; these cold/normal observations are **not** a comparable performance baseline, improvement claim or approved budget (ISC-62 remains open). An *earlier* `.8` probe saw two attached keys (25555459 and 31683481) and selected only 31683481; it must not be relabeled as the final probe's topology. No fresh smart-card probe accompanied the final FIDO/OTP run. The last broad Core smart-card hardware integration and isolated `--smartcard` child for 31683481 returned `SCARD_E_SHARING_VIOLATION` **before open**. No transaction or read completed on that smart-card attempt. The read-only smart-card scenario provides phase diagnostics under a 20-second watchdog; it does not prove an SDK API defect or clear the hardware cell. Earlier selected-key smart-card success remains historical/version-specific. No operator action or retry is requested.
