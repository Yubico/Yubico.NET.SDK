...
The latest selected-key `.8` Native AOT run at source `21498d24` passed all
eight read-only route modes listed in [local close-out evidence](#verification).
...
not passed. Matched final-source `.8` measurements support local D2 only;
native-only duration, pending worker counts and approved global budget remain
unavailable. The eight-mode probe ran after the public completion fix at the
same shipping source bytes recorded by `21498d24`; commit metadata alone did
not change those runtime bytes. Earlier `c417621b` measurements/probes are
historical, not the final-source comparison.
...
**Prior software checkpoint, historical:** At `8d13fd1e`/`8ebcd026`, Core 1,511 passed/3 skipped, PublicApi 22, Fido2 471, YubiOtp 180 and resilience-fast 88 passed; adapter registry 17, public-sync registry five, PC/SC lifetime 30 and HID scan boundary three passed. Twelve then-changed shipping methods were within cyclomatic 10/cognitive 20. The independent review was PASS WITH NOTES after correcting held-scan test cleanup. These are not the final-source regression counts. Public SELECT application-ID hex logging remains.

**Earlier selected-key checkpoint, historical:** Before the replug, broad Core smart-card integration and isolated `--smartcard` failed `SCARD_E_SHARING_VIOLATION` before open. Replug resolved that observed sharing contention; `c417621b` then passed three read-only PC/SC cycles. At `c417621b`, an initial probe revealed a *different* correctness defect: connection Dispose returned before the public pending-read task reached terminal completion because the async cancellation-registration Dispose wrapper had not finished. The probe was **not** weakened. `21498d24` fixed the public completion barrier with shared shutdown waiting and public read-task markers; four new controlled tests include two that failed on the old implementation. The final-source probe and regression results below supersede these earlier checkpoints, not erase them.
