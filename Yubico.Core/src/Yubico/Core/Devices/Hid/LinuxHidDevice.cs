...
internal class LinuxHidDevice : HidDevice
{
    ...
    private readonly ILogger _log = Log.GetLogger<LinuxHidDevice>();
    ...
    private static int GetDescriptorLength(LinuxFileSafeHandle handle)
    {
        ...
        {
            Log.GetLogger(typeof(LinuxHidDevice).FullName!).LogWarning(
                "IOCTL failed. {Error}", LibcHelpers.GetErrnoString());
        }
