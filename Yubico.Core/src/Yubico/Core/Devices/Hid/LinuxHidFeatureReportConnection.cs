    ...
    internal class LinuxHidFeatureReportConnection : IHidConnection
    {
        ...
        public void SetReport(byte[] report)
        {
            ...
            try
            {
                ...
                int status = NativeMethods.ioctl(_handle, ioctlFlag, setReportData);
                if (status >= 0)
                {
                    return;
                }
        ...
        public byte[] GetReport()
        {
            ...
            try
            {
                ...
                int status = NativeMethods.ioctl(_handle, ioctlFlag, getReportData);
                if (status >= 0)
                {
                    // A YubiKey "has a usable payload of 8 bytes". Hence,
                    // if we receive something longer than 8, just return the
                    // last 8.
                    byte[] outputBuffer = new byte[status];
                    Marshal.Copy(getReportData, outputBuffer, 0, status);
                    if (status > YubiKeyFeatureReportSize)
                    {
                        outputBuffer = (byte[])outputBuffer.Skip(status - YubiKeyFeatureReportSize);
                    }
                    return outputBuffer;
                }
            }
