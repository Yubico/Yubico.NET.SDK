...
namespace Yubico.Core.Logging
{
    ...
    /// <remarks>
    ///     <para>
    ///         This class builds on top of the standard <see cref="ILogger" /> interface used by the Microsoft.Extensions
    ///         logging
    ///         library. This is a meta-library for interoperating with different concrete logging implementations such as
    ///         NLog,
    ///         Serilog, or .NET's built in EventPipe system.
    ///     </para>
    ///     <para>
    ///         Methods for logging potentially sensitive information are present. These methods are disabled for Release
    ///         builds,
    ///         resulting in a no-op for anything other than a Debug build of this library.
    ///     </para>
    ///     <para>
    ///         Extension methods can be used to add further conveniences to the logging interface. For example, if you wanted
    ///         to
    ///         log a platform error code in a uniform way, you could introduce a `LogPlatformError` extension that takes care
    ///         of
