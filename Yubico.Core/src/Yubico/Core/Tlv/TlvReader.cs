...
namespace Yubico.Core.Tlv
{
    ...
    public sealed class TlvReader
    {
        ...
        {
            _ = CommonReadValue(out ReadOnlyMemory<byte> value, expectedTag, NoFixedLength, throwIfFailed: true);
        ...
        public bool TryReadValue(out ReadOnlyMemory<byte> value, int expectedTag) =>
            CommonReadValue(out value, expectedTag, NoFixedLength, throwIfFailed: false);
        ...
        {
            _ = CommonReadValue(out ReadOnlyMemory<byte> value, expectedTag, FixedLengthByte, throwIfFailed: true);
        ...
        {
            _ = CommonReadValue(out ReadOnlyMemory<byte> value, expectedTag, FixedLengthInt16, throwIfFailed: true);
        ...
        {
            _ = CommonReadValue(out ReadOnlyMemory<byte> value, expectedTag, FixedLengthInt32, throwIfFailed: true);
        ...
        public ReadOnlyMemory<byte> ReadEncoded(int expectedTag)
        {
            ...
            _ = CommonReadValue(out _, expectedTag, NoFixedLength, throwIfFailed: true);

            return _encoding[startOffset.._currentOffset];
        }
        ...
        public bool TryReadEncoded(out ReadOnlyMemory<byte> encoded, int expectedTag)
        {
            ...
            int startOffset = _currentOffset;
            if (CommonReadValue(out _, expectedTag, NoFixedLength, throwIfFailed: false))
            {
                encoded = _encoding[startOffset.._currentOffset];
                return true;
            }
        ...
        /// <summary>
        ///     Attempts to read and validate the expected tag, its length, and its value, indicating success through the return
        ///     value and providing a result code.
        /// </summary>
        ...
        ///     Out parameter that returns the result code of the operation, indicating the specific outcome
        ///     or error.
        /// </param>
        /// <returns>
        ///     Returns true if the tag, length, and value are successfully read and validated. Returns false if any
        ///     validation fails, along with the appropriate result code.
        /// </returns>
        ...
        private int ReadTag(int tagLength)
        {
            ...
            _currentTag = _encoding.Span[_currentOffset];
            for (int index = 1; index < tagLength; index++)
            {
                _currentTag <<= 8;
                _currentTag += _encoding.Span[_currentOffset + index];
            }
        ...
        private int ReadLength()
        {
            ...
            {
                _currentLength = _encoding.Span[_currentOffset + _currentTagLength];
            ...
            {
                _currentLength <<= 8;
                _currentLength += _encoding.Span[_currentOffset + _currentTagLength + index];
            }
        ...
        private static void ThrowOnFailedRead(int errorCode)
        {
            ...
            {
                UnsupportedTag => ExceptionMessages.TlvUnsupportedTag,
                UnsupportedLength => ExceptionMessages.TlvUnsupportedLengthField,
                UnexpectedEnd => ExceptionMessages.TlvUnexpectedEndOfBuffer,
                _ => ExceptionMessages.TlvUnexpectedEncoding
            };
