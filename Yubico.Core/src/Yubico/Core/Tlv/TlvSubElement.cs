...
internal class TlvSubElement : TlvEncoder
{
    ...
    public override bool TryEncode(Span<byte> encoding, int offset, out int bytesWritten)
    {
        ...
        Span<byte> destination = encoding[offset..];
        _tagAndLength.AsSpan().CopyTo(destination);

        destination = encoding[(offset + _tagAndLength.Length)..];
    ...
    /// <inheritdoc />
    public override void Clear() => CryptographicOperations.ZeroMemory(_value);
}
