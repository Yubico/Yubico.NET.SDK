...
public class HidTranslatorTests
{
    ...
    public void GetHidCodes_GivenKeyboardLayout_ReturnsCorrectInstance(
    ...
    public void GetHidCodes_GivenString_ReturnsCorrectCodes(
        KeyboardLayout layout,
        byte[] expected)
    {
        var hid = HidCodeTranslator.GetInstance(layout);
    ...
    [InlineData(KeyboardLayout.es_US, new byte[] { 0x9c, 0x18, 0x05, 0x0c, 0x06, 0x12, 0x9e })]
    [InlineData(KeyboardLayout.sv_SE, new byte[] { 0x9c, 0x18, 0x05, 0x0c, 0x06, 0x12, 0x9e })]
    public void GetHidCodes_GivenCharArray_ReturnsCorrectCodes(
        KeyboardLayout layout,
        byte[] expected)
    {
        var hid = HidCodeTranslator.GetInstance(layout);
    ...
    [InlineData(KeyboardLayout.es_US, new byte[] { 0x9c, 0x18, 0x05, 0x0c, 0x06, 0x12, 0x9e })]
    [InlineData(KeyboardLayout.sv_SE, new byte[] { 0x9c, 0x18, 0x05, 0x0c, 0x06, 0x12, 0x9e })]
    public void GetString_GivenHidCodes_ReturnsCorrectString(
        KeyboardLayout layout,
        byte[] input)
    {
        var expected = "Yubico!";
    ...
    [InlineData(KeyboardLayout.es_US)]
    [InlineData(KeyboardLayout.sv_SE)]
    public void TestSpecializedKeyboardSupportsModhexCodes(
        KeyboardLayout layout)
    {
    ...
    [InlineData(KeyboardLayout.es_US)]
    [InlineData(KeyboardLayout.sv_SE)]
    public void TestSpecializedKeyboardSupportsModhexChars(
        KeyboardLayout layout)
    {
    ...
    [InlineData(KeyboardLayout.es_US)]
    [InlineData(KeyboardLayout.sv_SE)]
    public void TestSpecializedKeyboardSupportsModhexString(
        KeyboardLayout layout)
    {
        // Since ModHex is a subset of all keyboard layouts, this will confirm
        // that all ModHex chars are the same in non-ModHex layouts.
...
#pragma warning restore CA1825
    ...
    private static (char, byte)[] GetDataForKeyboard(
        KeyboardLayout layout)
    {
        ...
        foreach (var code in codes)
        {
            ...
            var vkey = NativeMethods.MapVirtualKeyEx(scanCode, 1, hLayout);

            // I don't know why ToAsciiEx needs both the VKEY and the scan
            // code, but it does.
            var result = NativeMethods.ToAsciiEx(vkey, scanCode, keyboardState, output, 0, hLayout);
            if (result != 1)
            {
    ...
    private static IntPtr GetKeyboardLayout(
        KeyboardLayout layout)
    {
