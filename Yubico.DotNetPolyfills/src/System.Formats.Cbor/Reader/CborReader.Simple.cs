    ...
    public partial class CborReader
    {
        ...
        public float ReadSingle()
        {
            ...
            switch (header.AdditionalInfo)
            {
                ...
                case CborAdditionalInfo.Additional32BitData:
                    ...
                    result = ReadSingleBigEndian(buffer.Slice(1));
                    AdvanceBuffer(1 + sizeof(float));
                    AdvanceDataItemCounters();
                    return result;

                case CborAdditionalInfo.Additional64BitData:
                    throw new InvalidOperationException(CborExceptionMessages.Cbor_Reader_ReadingDoubleAsSingle);

                default:
                    throw new InvalidOperationException(CborExceptionMessages.Cbor_Reader_NotAFloatEncoding);
        ...
        public double ReadDouble()
        {
            ...
            switch (header.AdditionalInfo)
            {
                ...
                case CborAdditionalInfo.Additional64BitData:
                    ...
                    result = ReadDoubleBigEndian(buffer.Slice(1));
        ...
        public bool ReadBoolean()
        {
            ...
            bool result = header.AdditionalInfo switch
            {
                ...
                _ => throw new InvalidOperationException(CborExceptionMessages.Cbor_Reader_NotABooleanEncoding),
            };
        ...
        public void ReadNull()
        {
            ...
            switch (header.AdditionalInfo)
            {
                case (CborAdditionalInfo)CborSimpleValue.Null:
                    ...
                    return;
                default:
                    throw new InvalidOperationException(CborExceptionMessages.Cbor_Reader_NotANullEncoding);
            }
        }
        ...
        public CborSimpleValue ReadSimpleValue()
        {
            ...
            switch (header.AdditionalInfo)
            {
                ...
                case CborAdditionalInfo.Additional8BitData:
                    ...
                    {
                        throw new CborContentException(string.Format(CultureInfo.CurrentCulture, CborExceptionMessages.Cbor_ConformanceMode_InvalidSimpleValueEncoding, ConformanceMode));
                    }
                ...
                default:
                    throw new InvalidOperationException(CborExceptionMessages.Cbor_Reader_NotASimpleValueEncoding);
            }
        }
        ...
        private static double ReadHalfBigEndian(ReadOnlySpan<byte> buffer)
        {
            ...
            if (exp == 0)
            {
                value = mant * Math.Pow(2, -24); // ScaleB(mant, -24)
            }
            else if (exp != 31)
            {
                value = (mant + 1024) * Math.Pow(2, exp - 25); // ScaleB(mant + 1024, exp - 25)
            }
            else
            {
        ...
        private static byte[] AsReversedArray(ReadOnlySpan<byte> inp)
        {
            byte[] arr = inp.ToArray();
            MemoryExtensions.Reverse<byte>(arr);
        ...
        private static float ReadSingleBigEndian(ReadOnlySpan<byte> buffer)
        {
            ...
            {
                return BitConverter.ToSingle(AsReversedArray(buffer), 0);
            } 
            else
            {
        ...
        private static double ReadDoubleBigEndian(ReadOnlySpan<byte> buffer)
        {
            buffer = buffer[..8];
            if (BitConverter.IsLittleEndian)
            {
                return BitConverter.ToDouble(AsReversedArray(buffer), 0);
            }
            else
            {
