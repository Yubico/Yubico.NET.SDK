    ...
    public partial class CborReader
    {
        ...
        public byte[] ReadByteString()
        {
            ...
            if (header.AdditionalInfo == CborAdditionalInfo.IndefiniteLength)
            {
                ...
                {
                    throw new CborContentException(string.Format(CultureInfo.CurrentCulture, CborExceptionMessages.Cbor_ConformanceMode_IndefiniteLengthItemsNotSupported, ConformanceMode));
                }
        ...
        public void ReadStartIndefiniteLengthByteString()
        {
            ...
            if (header.AdditionalInfo != CborAdditionalInfo.IndefiniteLength)
            {
                throw new InvalidOperationException(CborExceptionMessages.Cbor_Reader_NotIndefiniteLengthString);
            }
        ...
        public string ReadTextString()
        {
            ...
            try
            {
                result = utf8Encoding.GetString(encodedString.ToArray());
            }
            catch (DecoderFallbackException e)
            {
                throw new CborContentException(CborExceptionMessages.Cbor_Reader_InvalidCbor_InvalidUtf8StringEncoding, e);
            }
        ...
        public bool TryReadTextString(Span<char> destination, out int charsWritten)
        {
            ...
            utf8Encoding.GetChars(encodedSlice.ToArray()).CopyTo(destination);
        ...
        public ReadOnlyMemory<byte> ReadDefiniteLengthTextStringBytes()
        {
            ...
            if (_isConformanceModeCheckEnabled && CborConformanceModeHelpers.RequiresUtf8Validation(ConformanceMode))
            {
                ...
                _ = ValidateUtf8AndGetCharCount(encodedSlice.Span, encoding);
            }
        ...
        private byte[] ReadIndefiniteLengthByteStringConcatenated()
        {
            ...
            byte[] output = new byte[concatenatedBufferSize];
        ...
        private string ReadIndefiniteLengthTextStringConcatenated()
        {
            ...
            ReadOnlySpan<byte> source = _data.Slice(_offset).Span;
            var output = new StringBuilder(concatenatedStringSize);

            foreach ((int o, int l) in ranges)
            {
                _ = output.Append(utf8Encoding.GetChars(source.Slice(o, l).ToArray()));
            }
        ...
        {
            List<(int Offset, int Length)> ranges = ReadIndefiniteLengthStringChunkRanges(CborMajorType.TextString, out int encodingLength, out _);
            Encoding utf8Encoding = CborConformanceModeHelpers.GetUtf8Encoding(ConformanceMode);
            ReadOnlySpan<byte> buffer = GetRemainingBytes();
            ...
            foreach ((int o, int l) in ranges)
            {
                utf8Encoding.GetChars(buffer.Slice(o, l).ToArray()).CopyTo(destination);
                destination = destination.Slice(l);
            }
        ...
        private List<(int Offset, int Length)> ReadIndefiniteLengthStringChunkRanges(CborMajorType type, out int encodingLength, out int concatenatedBufferSize)
        {
            ...
            static CborInitialByte ReadNextInitialByte(ReadOnlySpan<byte> buffer, CborMajorType expectedType)
            {
                ...
                {
                    throw new CborContentException(CborExceptionMessages.Cbor_Reader_InvalidCbor_IndefiniteLengthStringContainsInvalidDataItem);
                }
        ...
        private void SkipString(CborMajorType type)
        {
                ...
                CborConformanceModeHelpers.RequiresUtf8Validation(ConformanceMode))
            {
                ReadOnlySpan<byte> encodedSlice = buffer.Slice(bytesRead, byteLength);
                Encoding utf8Encoding = CborConformanceModeHelpers.GetUtf8Encoding(ConformanceMode);
                _ = ValidateUtf8AndGetCharCount(encodedSlice, utf8Encoding);
            }
        ...
        private static int ValidateUtf8AndGetCharCount(ReadOnlySpan<byte> buffer, Encoding utf8Encoding)
        {
            try
            {
                return utf8Encoding.GetCharCount(buffer.ToArray());
            }
            catch (DecoderFallbackException e)
            {
                throw new CborContentException(CborExceptionMessages.Cbor_Reader_InvalidCbor_InvalidUtf8StringEncoding, e);
