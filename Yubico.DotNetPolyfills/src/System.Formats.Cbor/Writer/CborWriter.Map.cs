...
namespace System.Formats.Cbor
{
    ...
    public partial class CborWriter
    {
        ...
        public void WriteEndMap()
        {
            ...
            {
                throw new InvalidOperationException(CborExceptionMessages.Cbor_Writer_MapIncompleteKeyValuePair);
            }
        ...
        private void WriteStartMapDefiniteLength(int definiteLength)
        {
            ...
            _currentKeyOffset = BytesWritten;
        }
        ...
        private void HandleMapKeyWritten()
        {
            ...
            if (CborConformanceModeHelpers.RequiresUniqueKeys(ConformanceMode))
            {
                ...
                (int Offset, int Length) currentKey = (_currentKeyOffset!.Value, BytesWritten - _currentKeyOffset.Value);

                if (!keyEncodingRanges.Add(currentKey))
                {
                    // reset writer state to right before the offending key write
                    _buffer.AsSpan(currentKey.Offset, BytesWritten).Fill(0);
                    BytesWritten = currentKey.Offset;

                    throw new InvalidOperationException(string.Format(CultureInfo.CurrentCulture, CborExceptionMessages.Cbor_ConformanceMode_ContainsDuplicateKeys, ConformanceMode));
                }
            }

            // record the value buffer offset
            _currentValueOffset = BytesWritten;
        }
        ...
        private void HandleMapValueWritten()
        {
            ...
            if (CborConformanceModeHelpers.RequiresSortedKeys(ConformanceMode))
            {
                ...
                var currentKeyValueRange = new KeyValuePairEncodingRange(
                    ...
                    totalLength: BytesWritten - _currentKeyOffset.Value);
                ...
                if (!_keysRequireSorting && keyValuePairEncodingRanges.Count > 0)
                {
                    ...
                    KeyValuePairEncodingRange previousKeyValueRange = keyValuePairEncodingRanges[^1];
        ...
        private void CompleteMapWrite()
        {
            if (_keysRequireSorting)
            {
                ...
                int totalMapPayloadEncodingLength = BytesWritten - _frameOffset;
        ...
        private HashSet<(int Offset, int Length)> GetKeyEncodingRanges()
        {
            ...
            if (_pooledKeyEncodingRangeSets != null &&
                _pooledKeyEncodingRangeSets.Count > 0)
            {
                HashSet<(int Offset, int Length)>? result = _pooledKeyEncodingRangeSets.Pop();
                result.Clear();
        ...
        private List<KeyValuePairEncodingRange> GetKeyValueEncodingRanges()
        {
            ...
            if (_pooledKeyValuePairEncodingRangeLists != null &&
                _pooledKeyValuePairEncodingRangeLists.Count > 0)
            {
                List<KeyValuePairEncodingRange>? result = _pooledKeyValuePairEncodingRangeLists.Pop();
                result.Clear();
