    ...
    public partial class CborWriter
    {
        ...
        public void WriteByteString(ReadOnlySpan<byte> value)
        {
            ...
            if (ConvertIndefiniteLengthEncodings && _currentMajorType == CborMajorType.ByteString)
            {
                ...
                _currentIndefiniteLengthStringRanges!.Add((BytesWritten, value.Length));
            }

            value.CopyTo(_buffer.AsSpan(BytesWritten));
        ...
        public void WriteTextString(ReadOnlySpan<char> value)
        {
            ...
            try
            {
                length = utf8Encoding.GetByteCount(value.ToArray());
            }
            catch (EncoderFallbackException e)
            {
                throw new ArgumentException(CborExceptionMessages.Cbor_Writer_InvalidUtf8String, e);
            }
            ...
            _ = utf8Encoding.GetBytes(value.ToArray(), 0, value.Length, _buffer, BytesWritten);
            BytesWritten += length;
            AdvanceDataItemCounters();
        }
        ...
        private void PatchIndefiniteLengthString(CborMajorType type)
        {
            ...
            int initialOffset = BytesWritten;
            ...
            tempSpan.CopyTo(buffer.Slice(BytesWritten, definiteLength));
            BytesWritten += definiteLength;

            // clean up
            s_bufferPool.Return(tempBuffer);
            _currentIndefiniteLengthStringRanges.Clear();
            buffer[BytesWritten..initialOffset].Fill(0x0);
        }
    }
}
