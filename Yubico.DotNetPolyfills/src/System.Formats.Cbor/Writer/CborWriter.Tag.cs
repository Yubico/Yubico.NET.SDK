...
namespace System.Formats.Cbor
{
    public partial class CborWriter
    {
        ...
        public void WriteTag(CborTag tag)
        {
            ...
            {
                throw new InvalidOperationException(string.Format(CultureInfo.CurrentCulture, CborExceptionMessages.Cbor_ConformanceMode_TagsNotSupported, ConformanceMode));
            }
        ...
        public void WriteUnixTimeSeconds(double seconds)
        {
            ...
            {
                throw new ArgumentException(CborExceptionMessages.Cbor_Writer_ValueCannotBeInfiniteOrNaN, nameof(seconds));
            }
        ...
        public void WriteBigInteger(BigInteger value)
        {
            ...
            byte[] littleEndianEncoding = unsignedValue.ToByteArray();
            
            // NOTE: We replicate the .netstandard2.1 behavior for (isUnsigned: true, isBigEndian: true)
            IEnumerable<byte> unsignedBigEndianEncoding = littleEndianEncoding.Reverse();

            // The last byte is really 'padding' to indicate unsigned
            if (!unsignedValue.IsZero && littleEndianEncoding[^1] == 0x00) {
                unsignedBigEndianEncoding = unsignedBigEndianEncoding.Skip(1);
            }

            WriteTag(isUnsigned ? CborTag.UnsignedBigNum : CborTag.NegativeBigNum);
            WriteByteString(unsignedBigEndianEncoding.ToArray());
        }
        ...
        internal static class DecimalHelpers
        {
            ...
            public static void Deconstruct(decimal value, out decimal mantissa, out byte scale)
            {
                ...
                decimal.GetBits(value).CopyTo(buf);
            ...
            {
                Span<int> buf = stackalloc int[4];
                decimal.GetBits(mantissa).CopyTo(buf);
            ...
            public static decimal Reconstruct(decimal mantissa, long exponent)
            {
                ...
                {
                    throw new OverflowException(CborExceptionMessages.Cbor_Writer_DecimalOverflow);
                }
                else if (exponent >= 0)
                {
