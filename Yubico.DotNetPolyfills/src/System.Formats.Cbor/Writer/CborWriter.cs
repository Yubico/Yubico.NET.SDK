...
namespace System.Formats.Cbor
{
    ...
    public partial class CborWriter
    {
        ...
        public void Reset()
        {
            if (BytesWritten > 0)
            {
                Array.Clear(_buffer, 0, BytesWritten);
        ...
        public void WriteEncodedValue(ReadOnlySpan<byte> encodedValue)
        {
            ...
            var initialByte = new CborInitialByte(encodedValue[0]);
            ...
            {
                encodedValue.CopyTo(_buffer.AsSpan(BytesWritten));
                BytesWritten += encodedValue.Length;
            }

            AdvanceDataItemCounters();

            static void ValidateEncoding(ReadOnlySpan<byte> encodedValue, CborConformanceMode conformanceMode)
            {
                // copy value to get a Memory reference
                var memory = new Memory<byte>(encodedValue.ToArray());
                var reader = new CborReader(memory, conformanceMode: conformanceMode, allowMultipleRootLevelValues: false);
                ...
                {
                    throw new ArgumentException(CborExceptionMessages.Cbor_Writer_PayloadIsNotValidCbor, e);
                }
                finally
                {
                ...
                {
                    CryptographicOperations.ZeroMemory(memory.Span);
                    throw new ArgumentException(CborExceptionMessages.Cbor_Writer_PayloadIsNotValidCbor);
                }
        ...
        public int Encode(Span<byte> destination)
        {
            ...
            {
                throw new ArgumentException(CborExceptionMessages.Argument_EncodeDestinationTooSmall, nameof(destination));
            }
        ...
        private ReadOnlySpan<byte> GetSpanEncoding()
        {
            if (!IsWriteCompleted)
            {
                throw new InvalidOperationException(CborExceptionMessages.Cbor_Writer_IncompleteCborDocument);
            }

            return new ReadOnlySpan<byte>(_buffer, 0, BytesWritten);
        }
        ...
        private void EnsureWriteCapacity(int pendingCount)
        {
            ...
            if (_buffer is null || _buffer.Length - BytesWritten < pendingCount)
            {
                const int BlockSize = 1024;
                int blocks = checked(BytesWritten + pendingCount + (BlockSize - 1)) / BlockSize;
        ...
        private void PopDataItem(CborMajorType typeToPop)
        {
            ...
            if (typeToPop != _currentMajorType)
            {
                ...
                {
                    throw new InvalidOperationException(string.Format(CultureInfo.CurrentCulture, CborExceptionMessages.Cbor_PopMajorTypeMismatch, (int)_currentMajorType));
                }
                else
                {
            ...
            if (_isTagContext)
            {
                ...
                throw new InvalidOperationException(string.Format(CultureInfo.CurrentCulture, CborExceptionMessages.Cbor_PopMajorTypeMismatch, (int)CborMajorType.Tag));
            }

            if (_definiteLength - _itemsWritten > 0)
            {
                throw new InvalidOperationException(CborExceptionMessages.Cbor_NotAtEndOfDefiniteLengthDataItem);
            }
        ...
        private void WriteInitialByte(CborInitialByte initialByte)
        {
            if (_definiteLength - _itemsWritten == 0)
            {
                throw new InvalidOperationException(CborExceptionMessages.Cbor_Writer_DefiniteLengthExceeded);
            }
            ...
            switch (_currentMajorType)
            {
                ...
                case CborMajorType.TextString:
                    ...
                    {
                        throw new InvalidOperationException(CborExceptionMessages.Cbor_Writer_CannotNestDataItemsInIndefiniteLengthStrings);
                    }

                    break;
            }

            _buffer[BytesWritten++] = initialByte.InitialByte;
        }
        ...
        private void CompleteIndefiniteLengthWrite(CborMajorType type)
        {
            ...
            else
            {
                ...
                _buffer[BytesWritten++] = CborInitialByte.IndefiniteLengthBreakByte;
            }
        }
