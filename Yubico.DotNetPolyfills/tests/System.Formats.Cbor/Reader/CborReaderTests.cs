...
namespace System.Formats.Cbor.UnitTests
{
    public partial class CborReaderTests
    {
        ...
        {
            _ = Assert.Throws<ArgumentOutOfRangeException>(() => new CborReader(Array.Empty<byte>(), conformanceMode: mode));
        }
        ...
        public static void ReadEncodedValue_InvalidCbor_ShouldThrowCborContentException(string hexEncoding)
        {
            byte[] encoding = hexEncoding.HexToByteArray();
            var reader = new CborReader(encoding);
            _ = Assert.Throws<CborContentException>(() => reader.ReadEncodedValue());
        ...
        public static void ReadEncodedValue_InvalidConformance_ConformanceCheckEnabled_ShouldThrowCborContentException(CborConformanceMode mode, string hexEncoding)
        {
            ...
            _ = Assert.Throws<CborContentException>(() => reader.ReadEncodedValue(disableConformanceModeChecks: false));
            Assert.Equal(encoding.Length, reader.BytesRemaining);
        }
        ...
        {
            var q = new ECPoint() { X = hexExpectedQx.HexToByteArray(), Y = hexExpectedQy.HexToByteArray() };
            ...
            {
                var namedCurve = ECCurve.CreateFromFriendlyName(friendlyName);
