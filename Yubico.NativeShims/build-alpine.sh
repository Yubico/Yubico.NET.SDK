...
docker buildx build \
    --tag kindtek/yubico-safedb-alpine:sdb-0.1.4 \
    --file docker/Alpine/Dockerfile \
    --platform=linux/amd64 \
    --build-arg USER_ID=`id -u` \
    --build-arg GROUP_ID=`id -g` \
    --build-arg ARTIFACT_DIR=alpine-x64 \
    --output type=local,dest=alpine-x64 \
    .
...
docker buildx build \
    ...
    --build-arg USER_ID=`id -u` \
    --build-arg GROUP_ID=`id -g` \
    --build-arg ARTIFACT_DIR=alpine-x86 \
    --output type=local,dest=alpine-x86 \
    .
...
docker buildx build \
    ...
    --build-arg USER_ID=`id -u` \
    --build-arg GROUP_ID=`id -g` \
    --build-arg ARTIFACT_DIR=alpine-arm64 \
    --output type=local,dest=alpine-arm64 \
    .
