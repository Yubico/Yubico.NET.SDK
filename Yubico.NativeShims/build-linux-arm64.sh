...
sudo apt-get update -qq && sudo apt-get install cmake -yq
...
deb [arch=arm64] http://ports.ubuntu.com/ubuntu-ports/ focal-backports main restricted universe multiverse" | sudo tee /etc/apt/sources.list> /dev/null

sudo apt-get update -qq | DEBIAN_FRONTEND=noninteractive sudo apt-get install libpcsclite-dev:arm64 -yq
