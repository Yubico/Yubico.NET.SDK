...
internal static class HkdfUtilities
{
    ...
    int length = Sha256HashByteLength)
    {
        ...
        var pseudoRandomKey = HkdfExtract(salt, inputKeyMaterial);
        return HkdfExpand(
            pseudoRandomKey,
            contextInfo,
            length);
    ...
    private static ReadOnlyMemory<byte> HkdfExtract(
        ...
        int length)
    {
        ...
        for (byte index = 1; index <= numberOfBlocks; index++)
        {
            ...
            Array.Copy(currentBlock, 0, outputKeyMaterial, blockOffset, bytesToCopy);
