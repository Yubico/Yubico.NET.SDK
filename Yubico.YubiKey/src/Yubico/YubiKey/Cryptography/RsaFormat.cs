...
public static class RsaFormat
{
        ...
        out bool isVerified)
    {
        ...
        try
        {
            ...
            PerformMgf1(
                buffer, offsetHash, digest.Length, buffer, 0, psLength + digest.Length + 1, digestHashAlgorithm);
    ...
    public static byte[] FormatPkcs1Encrypt(ReadOnlySpan<byte> inputData, int keySizeBits)
    {
        ...
        while ((index = Array.FindIndex(buffer, 2, paddingLength, p => p == 0)) > 0)
        {
        ...
        out byte[] outputData)
    {
        ...
        try
        {
            ...
            {
                errorCount += digestHashAlgorithm.Hash[index] ^ buffer[index + digestLength + 1];
            }
    ...
    private static int BuildDigestInfo(ReadOnlySpan<byte> digest, int digestAlgorithm, Span<byte> buffer)
    {
        ...
        if (!isValid || outputLength != totalLength)
        {
            throw new ArgumentException(
                string.Format(
                    CultureInfo.CurrentCulture,
        ...
        out int algorithm)
    {
        ...
        if (isValid)
        {
            ...
            if (!sameOid || digest.Length != digestLength || oidParams.Length != 0)
            {
                returnValue = false;
            }
        }
