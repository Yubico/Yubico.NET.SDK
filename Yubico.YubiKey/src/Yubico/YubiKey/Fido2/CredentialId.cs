...
namespace Yubico.YubiKey.Fido2
{
    ...
    public class CredentialId : ICborEncode
    {
        ...
        ///      "id"          --byte string--
        ///      "type"        --text string--
        ///      "transports"  --sequence(string)-- (Optional)
        ///    }
        /// </code>
        /// </remarks>
        /// <param name="encodedCredentialId">
        /// The CBOR-encoded <c>credentialId</c>.
        /// </param>
        /// <exception cref="Ctap2DataException">
        /// The <c>encodedCredentialId</c> is not a correct encoding.
        /// </exception>
        public CredentialId(ReadOnlyMemory<byte> encodedCredentialId)
        {
            var cbor = new CborReader(encodedCredentialId, CborConformanceMode.Ctap2Canonical);
            ...
            try
            {
                while (count > 0)
                {
                    ...
                    switch (mapKey)
                    {
                        ...
                        case TagId:
                            Id = new ReadOnlyMemory<byte>(cbor.ReadByteString());
                            break;
                        ...
                        case TagTransports:
                            _transports = CborHelpers.ReadStringArray(cbor, _transports);
                            break;
                    }
