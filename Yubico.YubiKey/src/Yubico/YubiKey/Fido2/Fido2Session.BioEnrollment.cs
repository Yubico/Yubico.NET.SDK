...
public sealed partial class Fido2Session
{
    ...
    public BioModality GetBioModality()
    {
        ...
        int modality = response.Status == ResponseStatus.Success
            ? response.GetData()
            : 0;
    ...
    {
        var currentToken = GetAuthToken(false, PinUvAuthTokenPermissions.BioEnrollment);
        ...
        {
            currentToken = GetAuthToken(false, PinUvAuthTokenPermissions.BioEnrollment);
