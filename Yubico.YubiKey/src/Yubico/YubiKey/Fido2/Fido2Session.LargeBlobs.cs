...
public sealed partial class Fido2Session
{
    ...
    public SerializedLargeBlobArray GetSerializedLargeBlobArray()
    {
        ...
        var cborMap = new CborMap<int>(
            fullEncoding.GetBuffer().AsMemory(0, (int)fullEncoding.Length));
        ...
        try
        {
            ...
            do
            {
                var currentToken = GetAuthToken(
                    forceToken, PinUvAuthTokenPermissions.LargeBlobWrite);

                currentToken.CopyTo(token.AsMemory());

                int currentLength = remaining >= maxFragmentLength
                    ? maxFragmentLength
                    : remaining;
    ...
    private static byte[] BuildDataToAuth(byte[] inputData, int dataOffset, int dataLength, HashAlgorithm digester)
    {
        ...
        BinaryPrimitives.WriteInt32LittleEndian(dataSpan[(DataToAuthFfByteCount + 2)..], dataOffset);

        digest.CopyTo(dataSpan[DataToAuthPrefixLength..]);
