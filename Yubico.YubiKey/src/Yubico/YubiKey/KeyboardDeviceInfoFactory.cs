...
internal static class KeyboardDeviceInfoFactory
{
        ...
        [MaybeNullWhen(returnValue: false)] out YubiKeyDeviceInfo yubiKeyDeviceInfo)
    {
        try
        {
            ...
            if (yubiKeyDeviceInfo is not null)
            {
    ...
    private static bool TryGetSerialNumberFromKeyboard(IHidDevice device, out int? serialNumber)
    {
        try
        {
            ...
            Logger.LogError(
                "Reading serial number via the keyboard interface failed with: {Error} {Message}", response.StatusWord,
    ...
    private static void ErrorHandler(Exception exception, string message) => Logger.LogWarning(exception, message);
}
