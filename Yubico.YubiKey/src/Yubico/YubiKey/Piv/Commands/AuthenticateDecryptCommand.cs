...
///         RSA-2048 key, the block must be 256 bytes, for an RSA-3072 key, the block must be 384 bytes, and for an
///         RSA-4096 key, the block must be 512 bytes. If the actual data to decrypt
...
public sealed class AuthenticateDecryptCommand : AuthenticateCommand, IYubiKeyCommand<AuthenticateDecryptResponse>
{
    ...
    #region IYubiKeyCommand<AuthenticateDecryptResponse> Members

    /// <inheritdoc />
    public AuthenticateDecryptResponse CreateResponseForApdu(ResponseApdu responseApdu) => new(responseApdu);
