...
///         The PIN and PUK are both allowed to be 6 to 8 characters/bytes. The PIN can be any ASCII character. For
///         YubiKeys with firmware versions prior to 5.7, the PUK is allowed to be any character in the <c>0x00</c> -
///         <c>0xFF</c> range for a total length of 6-8 bytes. For YubiKeys with firmware version 5.7 and above, the PUK is
///         allowed to be any character in the <c>0x00</c> - <c>0x7F</c> range for a total length of 6-8 Unicode code
///         points.
...
public sealed class ChangeReferenceDataCommand : IYubiKeyCommand<ChangeReferenceDataResponse>
{
    ...
    public ChangeReferenceDataCommand(byte slotNumber, ReadOnlyMemory<byte> currentValue, ReadOnlyMemory<byte> newValue)
    {
        ...
        if (!PivPinUtilities.IsValidPinLength(currentValue.Length)
            || !PivPinUtilities.IsValidPinLength(newValue.Length))
        {
    ...
    #region IYubiKeyCommand<ChangeReferenceDataResponse> Members
    ...
    /// <inheritdoc />
    public ChangeReferenceDataResponse CreateResponseForApdu(ResponseApdu responseApdu) => new(responseApdu);
