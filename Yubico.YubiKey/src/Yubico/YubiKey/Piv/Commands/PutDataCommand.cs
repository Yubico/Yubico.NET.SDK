...
public sealed class PutDataCommand : IYubiKeyCommand<PutDataResponse>
{
    ...
    public ReadOnlyMemory<byte> EncodedData
    {
        ...
        set
        {
            if (!Tag.IsValidEncodingForPut(value))
            {
                throw new ArgumentException(
                    string.Format(
                        CultureInfo.CurrentCulture,
    ...
    #region IYubiKeyCommand<PutDataResponse> Members
    ...
    /// <inheritdoc />
    public PutDataResponse CreateResponseForApdu(ResponseApdu responseApdu) => new(responseApdu);
