...
public sealed class ResetRetryCommand : IYubiKeyCommand<ResetRetryResponse>
{
    ...
    {
        if (!PivPinUtilities.IsValidPinLength(puk.Length)
            || !PivPinUtilities.IsValidPinLength(newPin.Length))
        {
    ...
    #region IYubiKeyCommand<ResetRetryResponse> Members
    ...
    /// <inheritdoc />
    public ResetRetryResponse CreateResponseForApdu(ResponseApdu responseApdu) => new(responseApdu);
