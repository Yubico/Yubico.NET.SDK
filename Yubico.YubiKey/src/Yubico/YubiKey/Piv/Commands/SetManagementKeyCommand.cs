...
public sealed class SetManagementKeyCommand : IYubiKeyCommand<SetManagementKeyResponse>
{
    ...
    ///         <c>PivAlgorithm.Aes256</c>. FIPS YubiKeys versions 5.7 and greater require <c>PivAlgorithm.Aes192</c>. YubiKeys
    ///         with firmware versions prior to 5.4.2 can only use <c>PivAlgorithm.TripleDes</c>.
    ///     </para>
    ///     <para>
    ///         Note that you need to authenticate the current PIV management key before
    ///         setting it to a new value with this command.
    ...
    ///     <c>TripleDes</c> for keys with firmware 5.6.x and earlier and <c>Aes192</c> for YubiKeys with firmware 5.7.x and
    ///     later.
    /// </summary>
    public PivAlgorithm Algorithm { get; set; }

    #region IYubiKeyCommand<SetManagementKeyResponse> Members
    ...
    /// <inheritdoc />
    public SetManagementKeyResponse CreateResponseForApdu(ResponseApdu responseApdu) => new(responseApdu);
    ...
    {
        int tag = ((int)Algorithm << 8) + (PivSlot.Management & 0xFF);
