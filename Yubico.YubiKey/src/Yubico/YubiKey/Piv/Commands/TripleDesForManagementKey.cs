...
internal sealed class TripleDesForManagementKey : ISymmetricForManagementKey
{
    ...
    #region ISymmetricForManagementKey Members
        ...
        int outputOffset)
    {
        ...
        if (_cryptoTransformA is not null && _cryptoTransformB is not null)
        {
    ...
    {
        if (keyData[..ValidDesKeyLength].SequenceEqual(keyData.Slice(KeyOffsetSecond, ValidDesKeyLength)))
        {
            return EqualKeyOneAndTwo;
        }
