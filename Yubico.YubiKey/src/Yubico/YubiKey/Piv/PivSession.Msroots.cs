...
public sealed partial class PivSession : IDisposable
{
    ...
    private void WriteMsrootsSpan(ReadOnlySpan<byte> contents, int maxLength)
    {
        ...
        for (int index = 0; index < MsrootsObjectCount; index++)
        {
            ...
            if (!tlvWriter.TryEncode(encoding.Span, out int bytesWritten))
            {
            ...
            var command = new PutDataCommand(MsrootsTag + index, encoding[..bytesWritten]);
    ...
    public byte[] ReadMsroots()
    {
        ...
        int count = 0;
        var contentArray =
            (ReadOnlyMemory<byte>[])Array.CreateInstance(typeof(ReadOnlyMemory<byte>), MsrootsObjectCount);
