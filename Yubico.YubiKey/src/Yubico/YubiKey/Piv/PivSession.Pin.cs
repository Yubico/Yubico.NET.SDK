...
public sealed partial class PivSession : IDisposable
{
    ...
    public void ChangePinAndPukRetryCounts(byte newRetryCountPin, byte newRetryCountPuk)
    {
        Logger.LogInformation(
            "Change the PIV PIN and PUK retry counts: {PinCount}, {PukCount}.", newRetryCountPin,
            newRetryCountPuk);
        ...
        out int? retriesRemaining)
    {
        ...
        {
            if (ManagementKeyAuthenticated || TryAuthenticateManagementKey(managementKey))
            {
    ...
    ///     later, to change it again.
    ///     <para>
    ///         The PUK must be 6-8 characters. For YubiKeys with firmware versions prior to 5.7, the PUK is allowed to be any
    ///         character in the <c>0x00</c> - <c>0xFF</c> range for a total length of 6-8 bytes. For YubiKeys with firmware
    ///         version 5.7 and above, the PUK is allowed to be any character in the <c>0x00</c> - <c>0x7F</c> range for a
    ///         total length of 6-8 Unicode code points. The
    ...
    public void ChangePuk()
    {
        ...
        if (!TryChangeReference(KeyEntryRequest.ChangePivPuk, ChangePinOrPuk, PivPinOnlyMode.None))
        {
            throw new OperationCanceledException(
                string.Format(
                    CultureInfo.CurrentCulture,
    ...
    /// </exception>
    public bool TryChangePuk(
        ReadOnlyMemory<byte> currentPuk,
    ...
    public void ResetPin()
    {
        ...
        if (!TryChangeReference(KeyEntryRequest.ResetPivPinWithPuk, ResetPin, PivPinOnlyMode.None))
        {
            throw new OperationCanceledException(
                string.Format(
                    CultureInfo.CurrentCulture,
