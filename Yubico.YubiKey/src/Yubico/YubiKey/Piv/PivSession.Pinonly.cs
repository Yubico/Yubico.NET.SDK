...
public sealed partial class PivSession : IDisposable
{
    ...
    ///         will be default management key algorithm (Firmware 5.7.x and later: AES-192. Firmware 5.6.x and earlier:
    ///         TDES.), even if the current management key is a different
    ...
    ///     not of a valid key algorithm type (Firmware 5.7.x and later: AES-192. Firmware 5.6.x and earlier: TDES.), the data
    ///     stored on the YubiKey is
    ...
    public void SetPinOnlyMode(PivPinOnlyMode pinOnlyMode) =>
        SetPinOnlyMode(pinOnlyMode, DefaultManagementKeyAlgorithm);
        ...
        PivAlgorithm mgmtKeyAlgorithm)
    {
        ...
        if (pinOnlyCheck == PinOnlyCheck.CanContinue)
        {
            currentPinOnlyMode |= GetPinDerivedStatus(
                adminData, currentPinOnlyMode.HasFlag(PivPinOnlyMode.PinProtected), specialKeyCollector,
                userKeyCollector);
        ...
        if (newPinOnlyMode.HasFlag(PivPinOnlyMode.PinDerived))
        {
            ...
            SetYubiKeyPinDerived(
                adminData, currentPinOnlyMode, mgmtKeyAlgorithm, specialKeyCollector, userKeyCollector,
                ref newPinOnlyMode);
        }
    ...
    private void ClearPinOnly(PivPinOnlyMode currentMode, SpecialKeyCollector specialKeyCollector)
    {
        ...
        specialKeyCollector.SetKeyData(
            SpecialKeyCollector.SetKeyDataDefault, ReadOnlyMemory<byte>.Empty, true,
            managementKeyAlgorithm);
    ...
    private bool TryGetChangePinMode(ReadOnlyMemory<byte> pin, out PivPinOnlyMode mode, out int? retriesRemaining)
    {
        ...
        try
        {
            ...
            using (pinProtect)
            {
                if (isValid && pinProtect.ManagementKey is not null)
                {
    ...
    #region Nested type: MgmtKeyHolder
    ...
    private sealed class MgmtKeyHolder : IDisposable
    {
        ...
        public bool IsKeyDataWeak(PivAlgorithm algorithm)
        {
            ...
            {
                if (_keyData.Span[..8].SequenceEqual(_keyData.Span.Slice(8, 8))
                    || _keyData.Span.Slice(8, 8)
                        .SequenceEqual(_keyData.Span.Slice(16, 8)))
                {
                    return true;
                }
            }
    ...
    #region Nested type: PinOnlyCheck
    ...
    #region Nested type: SpecialKeyCollector
    ...
    private sealed class SpecialKeyCollector : IDisposable
    {
        ...
        {
            var destinationKeyHolder = isNewKey
                ? _newKey
                : _currentKey;
        ...
        public ReadOnlyMemory<byte> DeriveKeyData(ReadOnlyMemory<byte> salt, PivAlgorithm algorithm, bool isNewKey)
        {
            ...
            return destinationKeyHolder.DeriveKeyData(_pinMemory[.._pinLength], salt, algorithm);
        }
            ...
            Func<KeyEntryData, bool>? userKeyCollector)
        {
            ...
            try
            {
                ...
                {
                    SetKeyData(
                        SetKeyDataBuffer, keyEntryData.GetCurrentValue(), false,
        ...
        public bool KeyCollectorSpecial(KeyEntryData keyEntryData)
        {
            ...
            switch (keyEntryData.Request)
            {
                ...
                case KeyEntryRequest.VerifyPivPin:
                    keyEntryData.SubmitValue(_pinMemory[.._pinLength].Span);
