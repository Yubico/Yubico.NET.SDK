...
internal class Scp03State : ScpState
{
        ...
        ReadOnlyMemory<byte> hostChallenge)
    {
        ...
        initializeUpdateResponse.ThrowIfFailed(
            $"Error when performing {initializeUpdateCommand.GetType().Name}: {initializeUpdateResponse.StatusMessage}");
    ...
    private (ExternalAuthenticateCommand, ReadOnlyMemory<byte> macChainingValue) GetMacedExternalAuthenticateCommand(
        ExternalAuthenticateCommand externalAuthenticateCommand)
    {
        var (commandDataMaced, newMacChainingValue) =
            GetMacdCommandData(externalAuthenticateCommand.CreateCommandApdu());

        return (new ExternalAuthenticateCommand(commandDataMaced), newMacChainingValue);
    }

    private (ReadOnlyMemory<byte> commandDataMaced, ReadOnlyMemory<byte> newMacChainingValue) GetMacdCommandData(
