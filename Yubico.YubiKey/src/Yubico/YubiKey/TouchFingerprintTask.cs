...
namespace Yubico.YubiKey
{
    ...
    // While the SDK is doing its work, it can look at the IsUserCanceled
    // property every so often to see if the user has canceled.
    ...
    // and then quit.
    internal class TouchFingerprintTask
    {
        ...
        // The SDK will check this property every now and then to see if the user
        // has canceled.
        ...
        // write to this property if the user cancels.
        ...
        public TouchFingerprintTask(Func<KeyEntryData, bool> keyCollector, KeyEntryData keyEntryData)
        {
            ...
            var notifyTask = new Task(() => RunKeyCollectorTask(keyCollector));
            _ = notifyTask.ContinueWith((t) => HandleTaskException(t), TaskScheduler.Current);
            notifyTask.Start();
        }

        private void HandleTaskException(Task task) =>
            task.Exception?.Handle((e) => { IsUserCanceled = true; return true; });
        ...
        public void SdkUpdate(KeyEntryData keyEntryData)
        {
            lock(_updateLock)
            {
                ...
                _keyEntryData = new KeyEntryData(UserCancel)
                {
                    Request = (keyEntryData.Request == KeyEntryRequest.EnrollFingerprint)
