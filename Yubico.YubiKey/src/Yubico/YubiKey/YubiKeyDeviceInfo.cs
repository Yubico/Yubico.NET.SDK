...
public class YubiKeyDeviceInfo : IYubiKeyDeviceInfo
{
    ...
    #region IYubiKeyDeviceInfo Members
    ...
    public string VersionName =>
        VersionQualifier.Type == VersionQualifierType.Final
    ...
    private static void SetSkySeries(YubiKeyDeviceInfo deviceInfo, bool skySeriesFlag) =>
    ...
    private static void SetFirmwareVersionAndQualifier(
        Dictionary<int, ReadOnlyMemory<byte>> responseApduData,
        YubiKeyDeviceInfo deviceInfo)
    {
        if (!responseApduData.TryGetValue(
                YubikeyDeviceManagementTags.VersionQualifierTag, out var versionQualifierBytes))
        {
            deviceInfo.VersionQualifier = new VersionQualifier(
                deviceInfo.FirmwareVersion, VersionQualifierType.Final, 0);
        }
        else
        {
        ...
        if (!isFinalVersion)
        {
            ...
            logger.LogDebug(
                "Overriding behavioral version with {FirmwareString}", deviceInfo.VersionQualifier.FirmwareVersion);
        }
