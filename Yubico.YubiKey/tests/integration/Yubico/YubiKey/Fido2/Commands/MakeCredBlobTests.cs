...
public class MakeCredBlobTests : NeedPinToken
{
    ...
    private readonly RelyingParty _rp = new("SomeRpId")
    {
        Name = "SomeRpName"
    };

    private readonly UserEntity _user = new(new ReadOnlyMemory<byte>(new byte[] { 0x11, 0x22, 0x33, 0x44 }))
    {
        Name = "SomeUserName",
        DisplayName = "User"
    };
    ...
    public void MakeCredentialBlob_Succeeds()
    {
        ...
        if (cData.AuthenticatorData.CredentialPublicKey is not null)
        {
    ...
    private bool CheckCredBlob(
        GetAssertionData aData)
    {
