...
public class MakeLargeBlobTests : NeedPinToken
{
    ...
    public void InitialLargeBlob_Succeeds()
    {
        ...
        if (authInfo.Options is not null)
        {
    ...
    public void SetLargeBlob_Succeeds()
    {
        ...
        var dataPlusDigest = BuildDataPlusDigest(dataToStore, 0, dataToStore.Length, digester);
        var offset = 0;
        var totalLength = dataToStore.Length;
        var dataToAuth = BuildDataToAuth(dataPlusDigest, 0, dataPlusDigest.Length, offset, digester);
    ...
    private byte[] BuildDataPlusDigest(
        byte[] inputData,
        int dataOffset,
        int dataLength,
