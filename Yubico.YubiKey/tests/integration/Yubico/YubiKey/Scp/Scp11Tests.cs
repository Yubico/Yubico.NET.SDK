...
public class Scp11Tests
{
    ...
    {
        return IntegrationTestDeviceEnumeration.GetTestDevice(desiredDeviceType, transport,
        ...
        StandardTestDevice desiredDeviceType)
    {
        ...
        using (var session = new SecurityDomainSession(testDevice, scp03KeyParams))
        {
            ...
            session.DeleteKey(new KeyReference(ScpKeyIds.Scp11B, 0x01));
        ...
        ReadOnlyMemory<char> ocePassword)
    {
        ...
        if (privateKeyEntry.Key is not ECPrivateKeyParameters ecPrivateKey)
        {
    ...
    private static ECParameters ConvertToECParameters(
        ECPrivateKeyParameters bcPrivateKey)
    {
        ...
        KeyReference keyReference)
    {
        ...
        var leaf = certificateList.Last();
        var ecDsaPublicKey = leaf.PublicKey.GetECDsaPublicKey()!;
        var keyParams = new Scp11KeyParameters(keyReference,
            ECPublicKey.CreateFromParameters(ecDsaPublicKey.ExportParameters(false)));
