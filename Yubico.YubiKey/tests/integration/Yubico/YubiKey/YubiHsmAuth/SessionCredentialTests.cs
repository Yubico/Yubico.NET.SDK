...
public class SessionCredentialTests
{
    ...
    public void AddCredential_DefaultTestCred_AppContainsOneCred(
        StandardTestDevice selectedDevice,
        bool useScp = false)
    {
        // Preconditions
        var testDevice = YhaTestUtilities.GetCleanDevice(selectedDevice);

        IReadOnlyList<CredentialRetryPair> credentialList;

        // Test
        using (var yubiHsmAuthSession =
               new YubiHsmAuthSession(testDevice, useScp ? Scp03KeyParameters.DefaultKey : null))
        {
