...
namespace Yubico.YubiKey.TestApp
{
    internal class Program : IOutput, IDisposable
    {
        ...
        //         on the same line.
        //     2b. Get the width of the console so that we can intelligently wrap
        ...
        private void Usage(bool asError = false)
        {
            ...
            foreach (var (id, desc) in paramUsage)
            {
        ...
        private void ProcessParameters()
        {
            ...
            foreach (var parameter in plugin.Parameters.Values.Where(p => p.Required))
            {
                ...
                {
                    throw new ArgumentException(
                        $"Required parameter [-{parameter.Shortcut} ({parameter.Name})] was not specified");
                }
            }
