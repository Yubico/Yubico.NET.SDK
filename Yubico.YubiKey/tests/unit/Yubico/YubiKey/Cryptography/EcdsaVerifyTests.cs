...
public class EcdsaVerifyTests
{
    ...
    public void PivKey_VerifyDigestedData_Succeeds()
    {
        ...
        var isVerified = verifier.VerifyDigestedData(digest, signature);
    ...
    public void CoseKey_VerifyDigestedData_WithMultipleCurves_Succeeds(
        KeyType keyType)
    {
        // Arrange
        var (ecCurve, coseCurve) = GetCurves(keyType);
    ...
    [InlineData(KeyType.ECP384)]
    [InlineData(KeyType.ECP521)]
    public void ECDsa_VerifyDigestedData_WithIeeeFormat_Succeeds(
        KeyType keyType)
    {
        // Arrange
    ...
    [InlineData(KeyType.ECP384)]
    [InlineData(KeyType.ECP521)]
    public void ECDsa_VerifyDigestedData_WithDerFormat_Succeeds(
        KeyType keyType)
    {
        // Arrange
        var (ecCurve, _) = GetCurves(keyType);
    ...
    [InlineData(KeyType.ECP384)]
    [InlineData(KeyType.ECP521)]
    public void ECDsa_VerifyData_WithDerFormat_Succeeds(
        KeyType keyType)
    {
        // Arrange
        var (ecCurve, _) = GetCurves(keyType);
        ...
        // Act
        using var verifier = new EcdsaVerify(pubKey);
        var isVerified = verifier.VerifyData(dataToSign, signature);
    ...
    private byte[] GetEncodedPoint()
    {
        ...
        var encoding = new byte[xCoord.Length + yCoord.Length + 1];
        encoding[0] = 0x4;
    ...
    private static (ECCurve ecCurve, CoseEcCurve coseCurve) GetCurves(
        KeyType keyType)
    {
