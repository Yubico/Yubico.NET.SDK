...
{
    private readonly long[] _longValues = new[]
    {
        -1, 0x05,
        -7, 0x05,
        -23, 0x05,
    ...
    [InlineData(true)]
    [InlineData(false)]
    public void ReadIntegers_Success(
        bool isSigned)
    {
        var index = 0;
        bool isValid;
    ...
    private bool TestInt32(
        byte[] encoding,
        int flags)
    {
        var cborMap = new CborMap<int>(encoding);
        var isSigned = cborMap.ReadBoolean(1);
        var index = cborMap.ReadInt32(2);

        if ((flags & 0x01) != 0)
        {
            var getValue = cborMap.ReadInt32(3);
            if (isSigned)
            {
        ...
        int flags)
    {
        ...
        if ((flags & 0x02) != 0)
        {
            var getValue = cborMap.ReadUInt32(3);
            if (isSigned)
            {
                var expectedValue = _longValues[index];
                if (getValue == expectedValue)
        ...
        int flags)
    {
        ...
        if ((flags & 0x04) != 0)
        {
            var getValue = cborMap.ReadInt64(3);
            if (isSigned)
            {
                var expectedValue = _longValues[index];
                if (getValue == expectedValue)
        ...
        int flags)
    {
        ...
        if ((flags & 0x08) != 0)
        {
            var getValue = cborMap.ReadUInt64(3);
            if (isSigned)
            {
                var expectedValue = _longValues[index];
