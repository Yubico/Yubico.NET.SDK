...
{
    public class Protocol1Tests
    {
        [Fact]
        public void Encapsulate_Correct()
        {
            var encodedKey = GetEncodedPublicKey();

            var pubKey = new CoseEcPublicKey(encodedKey);
            Assert.Equal(CoseEcCurve.P256, pubKey.Curve);

            var p1 = new PinUvAuthProtocolOne();
            Assert.Equal(PinUvAuthProtocol.ProtocolOne, p1.Protocol);

            p1.Encapsulate(pubKey);

            Assert.NotNull(p1.AuthenticatorPublicKey);
            Assert.NotNull(p1.PlatformPublicKey);
            _ = Assert.NotNull(p1.EncryptionKey);
            _ = Assert.NotNull(p1.AuthenticationKey);
        }
        ...
        public void Encrypt_Correct()
        {
            ...
            var encryptedData = p1.Encrypt(dataToEncrypt, offset: 0, dataToEncrypt.Length);
            var decryptedData = p1.Decrypt(encryptedData, offset: 0, encryptedData.Length);

            var isValid = dataToEncrypt.AsSpan().SequenceEqual(decryptedData.AsSpan());
        ...
        {
            var dataToAuthenticate = new byte[]
            {
            ...
            var authValue = p1.Authenticate(dataToAuthenticate);

            Assert.Equal(expected: 16, authValue.Length);
        }
        ...
        private byte[] GetEncodedPublicKey()
        {
            ...
            var returnValue = new byte[encodedKey.Length];
            Array.Copy(encodedKey, sourceIndex: 0, returnValue, destinationIndex: 0, encodedKey.Length);
