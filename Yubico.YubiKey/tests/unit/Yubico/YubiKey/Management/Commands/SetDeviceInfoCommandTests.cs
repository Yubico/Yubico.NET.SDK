...
namespace Yubico.YubiKey.Management.Commands
{
    public class SetDeviceInfoCommandTests
    {
        ...
        [Fact]
        public void Application_Get_AlwaysReturnsManagement()
        {
            var command = new SetDeviceInfoCommand();

            Assert.Equal(YubiKeyApplication.Management, command.Application);
        }
        ...
        [Fact]
        public void CreateCommandApdu_Ins0x1C()
        {
            var setCommand = new SetDeviceInfoCommand();

            var commandApdu = setCommand.CreateCommandApdu();

            Assert.Equal(expected: 0x1c, commandApdu.Ins);
        }
        ...
        public void CreateCommandApdu_RestrictNfcPresent_EncodesCorrectTlv()
        {
            ...
            Assert.Equal(nfcRestrictedTag, data.Span[index: 1]);
        }
    }
}
