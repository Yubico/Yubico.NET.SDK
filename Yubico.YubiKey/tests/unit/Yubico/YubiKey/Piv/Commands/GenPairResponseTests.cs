...
#pragma warning restore CS8625
    ...
    [InlineData(0x11)]
    public void Constructor_InvalidSlot_ThrowsException(
        byte slotNumber)
    {
        var responseApdu = GetResponseApdu(ResponseStatus.Success, KeyType.ECP256);

        _ = Assert.Throws<ArgumentException>(() =>
            new GenerateKeyPairResponse(responseApdu, slotNumber, PivAlgorithm.Rsa2048));
    }
    ...
    public void GetData_InvalidKeyData_ThrowsException(
        KeyType keyType)
    {
        var responseApdu = GetBadDataResponseApdu(keyType);
    ...
    {
        var responseApdu = GetResponseApdu(responseStatus, KeyType.ECP384);
    ...
    public void Constructor_SetsStatusCorrectly(
        ResponseStatus responseStatus)
    {
        var responseApdu = GetResponseApdu(responseStatus, KeyType.ECP256);
    ...
    public void Constructor_SetsSlotNumCorrectly(
        byte slotNumber)
    {
        var responseApdu = GetResponseApdu(ResponseStatus.Success, KeyType.RSA2048);
    ...
    [InlineData(KeyType.ECP256)]
    [InlineData(KeyType.ECP384)]
    public void GetData_FailResponse_ThrowsException(
        KeyType keyType)
    {
        var responseApdu = GetResponseApdu(ResponseStatus.Failed, keyType);
    ...
    [InlineData(KeyType.ECP256)]
    [InlineData(KeyType.ECP384)]
    public void GetData_AuthenticationRequiredResponse_ThrowsException(
        KeyType keyType)
    {
        var responseApdu = GetResponseApdu(ResponseStatus.AuthenticationRequired, keyType);
        var response = new GenerateKeyPairResponse(responseApdu, 0x8E, keyType.GetPivAlgorithm());
    ...
    public void GetData_ReturnsCorrectData(
        KeyType keyType) // TODO Make test for CUrve keys
    {
        var responseApdu = GetResponseApdu(ResponseStatus.Success, keyType);
        var keyData = GetCorrectEncodingForAlgorithm(keyType);
...
#pragma warning restore CS0618 // Type or member is obsolete
    ...
    private static byte[] BuildResponseForAlgorithm(
        KeyType keyType)
    {
        var encoding = GetCorrectEncodingForAlgorithm(keyType);
    ...
    private static byte[] GetCorrectEncodingForAlgorithm(
        KeyType keyType)
    {
    ...
    private static ResponseApdu GetBadDataResponseApdu(
        KeyType keyType)
    {
        var responseData = keyType switch
        {
            KeyType.RSA2048 => new byte[]
            {
