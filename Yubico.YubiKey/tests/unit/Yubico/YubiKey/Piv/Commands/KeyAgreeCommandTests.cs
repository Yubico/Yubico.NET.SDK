...
namespace Yubico.YubiKey.Piv.Commands
{
    public class KeyAgreeCommandTests
    {
        ...
        {
            var pubKey = GetPublicKey(algorithm);
            var command = new AuthenticateKeyAgreeCommand(pubKey, slotNumber: 0x9A);
        ...
        public void FullConstructor_NullData_ThrowsException()
        {
            _ = Assert.Throws<ArgumentException>(() =>
                new AuthenticateKeyAgreeCommand(correspondentPublicKey: null, slotNumber: 0x87));
        }
        ...
        {
            _ = Assert.Throws<ArgumentException>(() =>
                new AuthenticateKeyAgreeCommand(correspondentPublicKey: null, slotNumber: 0x9A));
        }
        ...
        {
            var pubKey = GetPublicKey(PivAlgorithm.EccP256);
            if (badFlag >= 0)
            {
            ...
            _ = Assert.Throws<ArgumentException>(() => new AuthenticateKeyAgreeCommand(pubKey, slotNumber: 0x9A));
        }
        ...
        {
            var pubKey = GetPublicKey(PivAlgorithm.EccP256);
            var command = new AuthenticateKeyAgreeCommand(pubKey, slotNumber: 0x90);
        ...
        {
            var cmdApdu = GetKeyAgreeCommandApdu(slotNumber: 0x86, algorithm);
        ...
        {
            var cmdApdu = GetKeyAgreeCommandApdu(slotNumber: 0x90, algorithm);
        ...
        {
            var cmdApdu = GetKeyAgreeCommandApdu(slotNumber: 0x91, algorithm);
        ...
        {
            var cmdApdu = GetKeyAgreeCommandApdu(slotNumber, algorithm);
        ...
        {
            var cmdApdu = GetKeyAgreeCommandApdu(slotNumber: 0x94, algorithm);
        ...
        {
            var cmdApdu = GetKeyAgreeCommandApdu(slotNumber: 0x95, algorithm);
        ...
        {
            var prefix = GetKeyAgreeDataPrefix(algorithm);
            var pubKey = GetPublicKey(algorithm);
            var expected = new List<byte>(prefix);
            expected.AddRange(pubKey);

            var cmdApdu = GetKeyAgreeCommandApdu(slotNumber: 0x9C, algorithm);
        ...
        private static byte[] GetPublicKey(PivAlgorithm algorithm)
        {
        ...
        private static byte[] GetKeyAgreeDataPrefix(PivAlgorithm algorithm)
        {
            return algorithm switch
            {
