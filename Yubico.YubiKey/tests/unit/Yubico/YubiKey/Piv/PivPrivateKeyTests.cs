...
namespace Yubico.YubiKey.Piv
{
    public class PivPrivateKeyTests
    {
        ...
        {
            var keyData = SampleKeyPairs.GetPivPrivateKey(algorithm).EncodedPrivateKey;
        ...
        public void Create_SetsEncodedCorrectly(PivAlgorithm algorithm)
        {
            ...
            var getKeyData = keyObject.EncodedPrivateKey;

            var compareResult = encoding.Span.SequenceEqual(getKeyData.Span);
        ...
        {
            ReadOnlyMemory<byte> keyData = GetKeyData(algorithm);
            ReadOnlyMemory<byte> primeP = GetRsaComponent(algorithm, tag: 1);
            ...
            var getPrimeP = rsaObject.PrimeP;

            var compareResult = primeP.Span.SequenceEqual(getPrimeP);
        ...
        {
            ReadOnlyMemory<byte> keyData = GetKeyData(algorithm);
            ReadOnlyMemory<byte> primeQ = GetRsaComponent(algorithm, tag: 2);
            ...
            var rsaObject = (PivRsaPrivateKey)keyObject;
            var getPrimeQ = rsaObject.PrimeQ;

            var compareResult = primeQ.Span.SequenceEqual(getPrimeQ);
        ...
        {
            ReadOnlyMemory<byte> keyData = GetKeyData(algorithm);
            ReadOnlyMemory<byte> exponentP = GetRsaComponent(algorithm, tag: 3);
            ...
            var rsaObject = (PivRsaPrivateKey)keyObject;
            var getExponentP = rsaObject.ExponentP;

            var compareResult = exponentP.Span.SequenceEqual(getExponentP);
        ...
        {
            ReadOnlyMemory<byte> keyData = GetKeyData(algorithm);
            ReadOnlyMemory<byte> exponentQ = GetRsaComponent(algorithm, tag: 4);
            ...
            var rsaObject = (PivRsaPrivateKey)keyObject;
            var getExponentQ = rsaObject.ExponentQ;

            var compareResult = exponentQ.Span.SequenceEqual(getExponentQ);
        ...
        {
            ReadOnlyMemory<byte> keyData = GetKeyData(algorithm);
            ReadOnlyMemory<byte> coefficient = GetRsaComponent(algorithm, tag: 5);
            ...
            var rsaObject = (PivRsaPrivateKey)keyObject;
            var getCoefficient = rsaObject.Coefficient;

            var compareResult = coefficient.Span.SequenceEqual(getCoefficient);
        ...
        public void CreateEcc_SetsPrivateValueCorrectly(PivAlgorithm algorithm)
        {
            ...
            var getPrivateValue = eccObject.PrivateValue;

            var compareResult = privateValue.Span.SequenceEqual(getPrivateValue);
        ...
        public void RsaConstructor_Components_BuildsEncoding(PivAlgorithm algorithm)
        {
            ...
            var getEncoding = rsaPrivate.EncodedPrivateKey;

            var compareResult = encoding.Span.SequenceEqual(getEncoding.Span);
        ...
        public void EccConstructor_Components_BuildsEncoding(PivAlgorithm algorithm)
        {
            ...
            var getEncoding = eccPrivate.EncodedPrivateKey;
        ...
        {
            _ = Assert.Throws<ArgumentException>(() => PivPrivateKey.Create(encodedPrivateKey: null));
        }
        ...
        {
            _ = Assert.Throws<ArgumentException>(() => PivRsaPrivateKey.CreateRsaPrivateKey(encodedPrivateKey: null));
        }

        [Fact]
        public void RsaConstructor_NoPrimeP_ThrowsExcpetion()
        {
            var keyData = GetKeyData(PivAlgorithm.Rsa1024);
            var badData = keyData.Slice(start: 66);
        ...
        {
            _ = Assert.Throws<ArgumentException>(() => new PivEccPrivateKey(privateValue: null));
        }
        ...
        {
            var privateValue = GetCorrectEncoding(algorithm);
            privateValue = privateValue.Slice(start: 3);
        ...
        public void GetPivPrivateKey_FromPem(PivAlgorithm algorithm)
        {
            ...
            var isValid = GetPemKey(algorithm, out var pemKey);
            Assert.True(isValid);

            var b64EncodedKey = pemKey
                .Replace("-----BEGIN PRIVATE KEY-----\n", newValue: null)
                .Replace("\n-----END PRIVATE KEY-----", newValue: null);

            var encodedKey = Convert.FromBase64String(b64EncodedKey);
        ...
        private static Memory<byte> GetPrivateValue(PivAlgorithm algorithm)
        {
            ...
            var keyData = GetCorrectEncoding(algorithm);
            var privateValue = keyData.Slice(start: 2);
            return privateValue;
        }
        ...
        private static Memory<byte> GetRsaComponent(PivAlgorithm algorithm, int tag)
        {
            ...
            var encoding = GetCorrectEncoding(algorithm);
            return encoding.Slice(start, count);
        }

        private static Memory<byte> GetCorrectEncoding(PivAlgorithm algorithm)
        {
        ...
        private static Memory<byte> GetKeyData(PivAlgorithm algorithm)
        {
            return algorithm switch
            {
