...
public class ListCredentialsResponseTests
{
    ...
    [InlineData(0)]
    [InlineData(65)]
    public void GetData_InvalidElementSize_ThrowsMalformedException(
        int labelLength)
    {
        // Algo - None
        // Touch - false
    ...
    [InlineData(1)]
    [InlineData(64)]
    public void GetData_OneElement_MatchesExpectedCredRetryObject(
        int labelSize)
    {
        var expectedKeyType = CryptographicKeyType.Aes128;
        var expectedLabel = new string('a', labelSize);
    ...
    {
        var expectedKeyType = CryptographicKeyType.Aes128;
        var expectedLabel = new string('a', 3);
        var expectedTouch = false;
        byte expectedRetryCount = 0;
