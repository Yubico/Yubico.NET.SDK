...
public class YubiKeyApplicationExtensionsTests
{
    ...
    public void GetIso7816ApplicationId_ReturnsCorrectId(
        YubiKeyApplication application,
        byte[] expectedId)
    {
        var result = application.GetIso7816ApplicationId();
    ...
    public void GetById_ReturnsCorrectApplication(
        byte[] applicationId,
        YubiKeyApplication expectedApplication)
    {
        var result = applicationId.GetYubiKeyApplication();
    ...
    {
        var unknownId = new byte[] { 0x00, 0x11, 0x22, 0x33 };
        Assert.Throws<ArgumentException>(() => unknownId.GetYubiKeyApplication());
    }
}
