...
public class YubikeyDeviceInfoTests
{
    ...
    private static readonly VersionQualifier DefaultVersionQualifier =
        new(new FirmwareVersion(), VersionQualifierType.Final, 0);
    ...
    public void ParseVersionQualifier_VariousScenarios_ParsesCorrectly()
    {
        ...
        Assert.Equal(new VersionQualifier(new FirmwareVersion(3, 4, 3), VersionQualifierType.Final, 0),
            info.VersionQualifier);
    ...
    private static YubiKeyDeviceInfo InfoOfVersion(
        byte[]? versionBytes,
        byte[]? qualifierBytes)
    {
    ...
    private static YubiKeyDeviceInfo DefaultInfo()
    {
        return YubiKeyDeviceInfo.CreateFromResponseData([]);
    }

    private static YubiKeyDeviceInfo DeviceInfoFor(
        int tag,
        byte[] deviceInfoData,
    ...
    private static void SetVersionTag(
        int tag,
        FirmwareVersion? version,
        Dictionary<int, ReadOnlyMemory<byte>> tlvs)
    {
        var versionAsBytes = version is not null
    ...
    private static byte[] VersionToBytes(
        FirmwareVersion version)
    {
