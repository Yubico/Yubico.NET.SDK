...
internal class AesGcmPrimitivesOpenSsl : IAesGcmPrimitives
{
        ...
        ReadOnlySpan<byte> associatedData)
    {
        ...
        try
        {
            using SafeEvpCipherCtx ctx = PlatformInterop.Desktop.Cryptography.NativeMethods.EvpCipherCtxNew();

            int status = PlatformInterop.Desktop.Cryptography.NativeMethods.EvpAes256GcmInit(true, ctx, keyBytes, nonce.ToArray());
            int outputLength;
            ...
            if (status != 0)
            {
                ...
                status = PlatformInterop.Desktop.Cryptography.NativeMethods.EvpUpdate(
            ...
            if (status != 0)
            {
                ...
                status = PlatformInterop.Desktop.Cryptography.NativeMethods.EvpFinal(ctx, tagBytes, out outputLength);
                if (outputLength != 0)
                {
                    status = 0;
                }
            ...
            if (status != 0)
            {
                status = PlatformInterop.Desktop.Cryptography.NativeMethods.EvpCipherCtxCtrl(ctx, PlatformInterop.Desktop.Cryptography.NativeMethods.CtrlFlag.GetTag, AuthTagLength, tagBytes);
            }
        ...
        ReadOnlySpan<byte> associatedData)
    {
        ...
        try
        {
            ...
            int status = PlatformInterop.Desktop.Cryptography.NativeMethods.EvpAes256GcmInit(false, ctx, keyBytes, nonce.ToArray());
            ...
            if (status != 0)
            {
                ...
                status = PlatformInterop.Desktop.Cryptography.NativeMethods.EvpCipherCtxCtrl(ctx, PlatformInterop.Desktop.Cryptography.NativeMethods.CtrlFlag.SetTag, AuthTagLength, tagBytes);
            }
            if (status != 0)
            {
