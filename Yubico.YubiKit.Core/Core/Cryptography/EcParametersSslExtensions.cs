...
internal static class OpenSslExtensions
{
    ...
    {
        SafeEcGroup group = PlatformInterop.Desktop.Cryptography.NativeMethods.EcGroupNewByCurveName(parameters.Curve.ToSslCurveId());
        SafeEcPoint point = PlatformInterop.Desktop.Cryptography.NativeMethods.EcPointNew(group);

        using SafeBigNum bnX = PlatformInterop.Desktop.Cryptography.NativeMethods.BnBinaryToBigNum(parameters.Q.X);
        using SafeBigNum bnY = PlatformInterop.Desktop.Cryptography.NativeMethods.BnBinaryToBigNum(parameters.Q.Y);
        _ = PlatformInterop.Desktop.Cryptography.NativeMethods.EcPointSetAffineCoordinates(group, point, bnX, bnY);
