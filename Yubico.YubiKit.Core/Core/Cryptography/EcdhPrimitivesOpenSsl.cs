...
internal class EcdhPrimitivesOpenSsl : IEcdhPrimitives
{
    ...
    public ECParameters GenerateKeyPair(ECCurve curve)
    {
        ...
        using SafeBigNum privateValueBn = PlatformInterop.Desktop.Cryptography.NativeMethods.BnBinaryToBigNum(privateValueBinary);

        // Create the curve that the public point should reside on.
        using SafeEcGroup group = PlatformInterop.Desktop.Cryptography.NativeMethods.EcGroupNewByCurveName(curve.ToSslCurveId());
        using SafeEcPoint publicPoint = PlatformInterop.Desktop.Cryptography.NativeMethods.EcPointNew(group);

        // Compute where the public point should reside on the curve, based on the private key.
        int result = PlatformInterop.Desktop.Cryptography.NativeMethods.EcPointMul(
            group,
            publicPoint,
        ...
        using SafeBigNum xBn = PlatformInterop.Desktop.Cryptography.NativeMethods.BnNew();
        using SafeBigNum yBn = PlatformInterop.Desktop.Cryptography.NativeMethods.BnNew();
        result = PlatformInterop.Desktop.Cryptography.NativeMethods.EcPointGetAffineCoordinates(group, publicPoint, xBn, yBn);
        ...
        result = PlatformInterop.Desktop.Cryptography.NativeMethods.BnBigNumToBinaryWithPadding(xBn, xBinary);
        ...
        result = PlatformInterop.Desktop.Cryptography.NativeMethods.BnBigNumToBinaryWithPadding(yBn, yBinary);
    ...
    public byte[] ComputeSharedSecret(ECParameters publicKey, ReadOnlySpan<byte> privateValue)
    {
        ...
        using SafeEcPoint sharedPoint = PlatformInterop.Desktop.Cryptography.NativeMethods.EcPointNew(group);
        ...
        using SafeBigNum x = PlatformInterop.Desktop.Cryptography.NativeMethods.BnNew();
        using SafeBigNum y = PlatformInterop.Desktop.Cryptography.NativeMethods.BnNew();
        result = PlatformInterop.Desktop.Cryptography.NativeMethods.EcPointGetAffineCoordinates(group, sharedPoint, x, y);
        ...
        result = PlatformInterop.Desktop.Cryptography.NativeMethods.BnBigNumToBinaryWithPadding(x, sharedSecret);
