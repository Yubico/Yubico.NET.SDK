...
internal class AesGcmPrimitivesOpenSsl : IAesGcmPrimitives
{
    ...
    #region IAesGcmPrimitives Members
        ...
        ReadOnlySpan<byte> associatedData)
    {
        ...
        try
        {
            ...
            if (status != 0)
            {
                ...
                if (outputLength != dataToEncrypt.Length) status = 0;
            }
            ...
            if (status != 0)
            {
                ...
                if (outputLength != 0) status = 0;
            }

            if (status != 0)
                status = NativeMethods.EvpCipherCtxCtrl(ctx, NativeMethods.CtrlFlag.GetTag, AuthTagLength,
                    tagBytes);

            if (status == 0) throw new SecurityException("ExceptionMessages.AesGcmFailed");
        ...
        ReadOnlySpan<byte> associatedData)
    {
        ...
        try
        {
            ...
            if (status != 0)
            {
                ...
                if (outputLength != ciphertext.Length) status = 0;
            }
            ...
            if (status != 0)
            {
                ...
                status = NativeMethods.EvpCipherCtxCtrl(ctx, NativeMethods.CtrlFlag.SetTag, AuthTagLength,
                    tagBytes);
            }
