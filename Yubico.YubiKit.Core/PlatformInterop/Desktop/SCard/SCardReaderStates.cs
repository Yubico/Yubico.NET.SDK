...
namespace Yubico.YubiKit.Core.PlatformInterop.Desktop.SCard
{
    ...
    internal unsafe struct SCARD_READER_STATE
    {
        ...
        public static SCARD_READER_STATE Create(string readerName) =>
            new(Marshal.StringToHGlobalAnsi(readerName));
        public static SCARD_READER_STATE[] CreateMany(IEnumerable<string> readerNames) =>
            readerNames.Select(name => new SCARD_READER_STATE(Marshal.StringToHGlobalAnsi(name))).ToArray();
