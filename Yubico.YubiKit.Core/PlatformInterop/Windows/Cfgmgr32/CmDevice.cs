...
public class CmDevice
{
    ...
    public CmDevice(string devicePath)
    {
        ...
        InstanceId = (string?)CmPropertyAccessHelper.TryGetProperty(NativeMethods.CM_Get_Device_Interface_Property, devicePath, NativeMethods.DEVPKEY_Device_InstanceId)!;
    ...
    public CmDevice? Parent()
    {
        ...
        if (errorCode != NativeMethods.CmErrorCode.CR_SUCCESS && errorCode != NativeMethods.CmErrorCode.CR_NO_SUCH_DEVNODE)
        {
...
#pragma warning restore CA1846 // Prefer 'AsSpan' over 'Substring'
    ...
    private int LocateDevNode()
    {
        ...
        NativeMethods.CmErrorCode errorCode = NativeMethods.CM_Locate_DevNode(out int devNodeHandle, InstanceId, flags);
        if (errorCode != NativeMethods.CmErrorCode.CR_SUCCESS)
        {
            throw new PlatformApiException(
                "CONFIG_RET",
