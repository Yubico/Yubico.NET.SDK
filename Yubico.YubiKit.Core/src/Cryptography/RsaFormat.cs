...
//             var digestInfoLength = BuildDigestInfo(digest, digestAlgorithm, bufferAsSpan);
//
//             var paddingLength = (keySizeBits / 8) - (digestInfoLength + 3);
...
//         //     //         digest
//         //     bool isValid;
//         //     var digestInfo = formattedSignature[(index + 1)..].ToArray();
//         //
//         //     try
//         //     {
...
//             var psLength = buffer.Length - ((2 * digest.Length) + 2);
//             var offsetSalt = psLength + 1;
//             var offsetHash = offsetSalt + digest.Length;
//
...
//             var buffer = formattedSignature.ToArray();
//
//             try
//             {
//                 // the salt is the same length as the digest.
//                 // PS || 01 || salt || H || BC
//                 var psLength = buffer.Length - ((2 * digest.Length) + 2);
//                 var offsetHash = buffer.Length - (digest.Length + 1);
//
//                 if (psLength < 0)
//                 {
//                     return false;
...
//                 var index = Array.FindIndex<byte>(buffer, p => p != 0);
//
...
//                 //  00 00 00 00 00 00 00 00 || digest || salt || H
//                 var mPrimeLength = (2 * digest.Length) + 8;
...
//             var paddingLength = buffer.Length - (inputData.Length + 3);
//             using var randomObject = CryptographyProviders.RngCreator();
...
//             var index = formattedData.Length - 2;
//             var startIndex = 0;
//
//             for (; index >= 2; index--)
//             {
...
//             using var digestHashAlgorithm = GetHashAlgorithm(digestAlgorithm);
//
//             var digestLength = digestHashAlgorithm.HashSize / 8;
//
...
//             // Copy the data into a byte[], so we can change the data (unmask) and
//             // also pass it as an argument to the digester.
//             var buffer = formattedData.ToArray();
//
//             try
//             {
...
//             //      04 digestLength
//             //         digest
//             var totalLength = 10 + oid.Length + digestLength;
...
//                 }
//
//                 var sameOid = oid.Span.SequenceEqual(new Span<byte>(supportedOid));
//
