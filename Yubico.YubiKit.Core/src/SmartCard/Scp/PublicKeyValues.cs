...
internal abstract class PublicKeyValues
{
    ...
    public static PublicKeyValues FromPublicKey(AsymmetricAlgorithm publicKey) =>
        publicKey switch
        {
            ...
            _ => throw new ArgumentException($"Unsupported public key type: {publicKey.GetType().Name}",
                nameof(publicKey))
        };

    #region Nested type: Ec
    ...
    internal sealed class Ec : PublicKeyValues
    {
        ...
        public Ec(ECDiffieHellmanPublicKey publicKey)
        {
            ...
            var coordinateLength = parameters.Q.X!.Length;
        ...
        public static Ec FromEncodedPoint(ECCurve curve, ReadOnlySpan<byte> encodedPoint)
        {
            if (encodedPoint.Length < 3 || encodedPoint[0] != 0x04)
                throw new ArgumentException("Invalid encoded point format. Expected uncompressed point (0x04 + X + Y).",
                    nameof(encodedPoint));

            var coordinateLength = (encodedPoint.Length - 1) / 2;
            if ((encodedPoint.Length - 1) % 2 != 0)
                throw new ArgumentException("Invalid encoded point length.", nameof(encodedPoint));

            var x = encodedPoint.Slice(1, coordinateLength).ToArray();
            var y = encodedPoint.Slice(1 + coordinateLength, coordinateLength).ToArray();

            using var ecdh =
                ECDiffieHellman.Create(new ECParameters { Curve = curve, Q = new ECPoint { X = x, Y = y } });
        ...
        {
            var coordinateLength = (EncodedPoint.Length - 1) / 2;
            var x = EncodedPoint.AsSpan(1, coordinateLength).ToArray();
            var y = EncodedPoint.AsSpan(1 + coordinateLength, coordinateLength).ToArray();
