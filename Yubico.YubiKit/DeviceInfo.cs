...
public readonly record struct DeviceInfo
{
    ...
    public required Version FirmwareVersion { get; init; }
    public required VersionQualifier VersionQualifier { get; init; }
    public Version? FpsVersion { get; init; }
    public Version? StmVersion { get; init; }
    ...
    public static DeviceInfo CreateFromTlvs(IReadOnlyCollection<Tlv> tlvPages, Version? defaultVersion)
    {
            ...
            !fpsVersionBytes.SequenceEqual(new byte[3]))
        {
            ...
            fpsVersion = new Version(fpsVersionMajor, fpsVersionMinor, fpsVersionMPatch);
        }
            ...
            !stmVersionBytes.SequenceEqual(new byte[3]))
        {
            ...
            stmVersion = new Version(stmVersionMajor, stmVersionMinor, stmVersionMPatch);
        }
        ...
        defaultVersion ??= new Version(firmwareVersionMajor, firmwareVersionMinor, firmwareVersionMPatch);
    ...
    private static (Version, VersionQualifier) DetermineFirmwareVersion(
        Dictionary<int, ReadOnlyMemory<byte>> dict,
        Version defaultVersion)
    {
        if (!dict.TryGetValue(TAG_VERSION_QUALIFIER, out var versionQualifierBytes))
            return (defaultVersion, new VersionQualifier(defaultVersion, VersionQualifierType.Final, 0));
        ...
        var qualifierVersion = Version.FromBytes(firmwareVersionBytes);
