...
- **SCP semantics unchanged (and a doc bug fixed)**: SCP is only valid on SmartCard. Supplying `scpKeyParams` while a non-SmartCard transport is selected (including the default HID FIDO) throws `NotSupportedException` ("SCP is only supported on SmartCard protocols") from `ApplicationSession.InitializeProtocolAsync` — a **pre-existing Core contract**, not introduced here (single-HID + SCP threw before Phase 38 too). The old Fido2 doc claim that SCP is "ignored over HID" was factually wrong (the code throws) and was corrected. "SCP implies/auto-switches to SmartCard" remains a deferred follow-up; silently dropping SCP on HID was explicitly rejected (it would hide a requested secure channel).
