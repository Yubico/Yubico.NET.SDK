...
- **Migration**: pass `Encoding.UTF8.GetBytes(password)` and zero the resulting array in a `finally`, or use an owning buffer type that zeros on disposal. The per-method `<remarks>` on `IHsmAuthSession` restate this at the call site.
