...
## 2026-08-19 - Connection/session ownership enforcement and new raw access tier

- Analyzed range `2bf3364889cd479aa9c7a7fe83bfc80a51fdb255..d37ae99143fa6e7bf64b6c1c65477e2d555614e6` (131 commits, 486 changed files; the session-contention/raw-tiers hardening effort plus a PIV device-wide lease test-assertion fix). The automated `diff.patch` context truncated before reaching most `src/Core` files, so this update was grounded directly in current source (`src/Core/src/Devices/ConnectionInUseException.cs`, `DeviceConnectionRegistry.cs`, `src/Core/src/Sessions/ConnectionSessionGuard.cs`, `RawSmartCardSession.cs`, `RawFidoHidSession.cs`, `RawOtpHidSession.cs`, `YubiKeyConnectionExtensions.cs`, `ISmartCardProtocol.cs`) and in-repo architecture docs (`docs/architecture/connection-ownership-and-contention.md`, `docs/architecture/raw-access-tiers.md`) rather than the truncated diff.
- Added `connection-session-ownership-enforcement` (manual, high confidence): v2 now enforces in-process exclusivity that v1 did not have - one physical YubiKey admits one live connection, one connection admits one live `ApplicationSession`, and a second concurrent acquisition throws the new `ConnectionInUseException` at acquisition time instead of failing later on the wire. Added a "Connection and Session Ownership" note under Session Lifecycle in `v1-to-v2.md`.
- Added `raw-access-tier-sessions` (assisted, high confidence): this range introduced a new "Tier 1: Raw Sessions" access level that did not previously exist - `RawSmartCardSession`, `RawFidoHidSession`, and `RawOtpHidSession`, reached through `IYubiKey.CreateRawSmartCardSessionAsync(...)` / `CreateRawFidoHidSessionAsync(...)` / `CreateRawOtpHidSessionAsync(...)`. Updated the Manual Low-Level Command Cases section in `v1-to-v2.md` to point v1 code that built raw APDUs or HID reports by hand at these guarded sessions before falling back to the unguarded raw `IConnection` escape hatch.
- Corrected the existing `scp-protocol-construction` map entry: `ISmartCardProtocol` and `ProtocolFactory` became `internal` in this range (previously `ISmartCardProtocol` was public), so the earlier `ISmartCardProtocol.WithScpAsync(...)` extension method documented as the v2 pattern no longer exists as a caller-facing entry point. The supported path is now `RawSmartCardSession`'s SCP-establishing `CreateAsync` overload, directly or through `CreateRawSmartCardSessionAsync(scpKeyParameters, ...)`.
- Test-only and internal-implementation portions of this range (PIV device-wide lease contention test alignment, HID/PC/SC protocol overlap refusal, discovery registry internals, macOS/Windows platform HID access-mode fixes) are covered by the ownership-enforcement note above where they affect observable behavior; no separate mapping was added for test-file-only changes.
- Advanced `docs/migration/.state.yml` `last_analyzed_commit` to `d37ae99143fa6e7bf64b6c1c65477e2d555614e6`.
