...
| `Yubico.NativeShims` 1.16.1 (public/local default); 1.17.4-prerelease.20260825.3 (authenticated validation) | Core | **Stable live paths runtime-verified; prerelease package-side evidence covers seven RIDs and v2 consumer evidence covers `osx-arm64`.** The package contains no managed assembly for the dependency analyzer. Core reaches it through 23 live, blittable P/Invoke declarations. Experiments 3 and 4 verify the stable PC/SC and OpenSSL ARKG paths. Experiment 10 records the prerelease static-package and internal-feed evidence. |
...
- **Stable 1.16.1 deployment requires its shared sidecar.** Running the self-contained AOT binary
  outside its publish directory failed to resolve `libYubico.NativeShims.dylib`; running from the
  publish directory succeeded. This is the evidence behind the stable deployment guidance.
- **Static package wiring is implemented in the 1.17.4 prerelease candidate.** The package supplies a
  merged NativeShims/OpenSSL archive and AOT-only MSBuild targets, removing the NativeShims sidecar
  and its working-directory sensitivity. Experiment 10 records the local evidence, the seven-RID
  package-side evidence, and the `osx-arm64` internal-feed consumer evidence.
...
- Status: **Closed.** `verification/NativeAotVerification` references reachable entry types from
  all ten supported libraries and runs Core discovery. A separate candidate-backed invocation
  directly probes NativeShims.
...
- Status: **Resolved by implementation and prerelease validation; stable release pending.** The
  candidate job in `.github/workflows/native-aot.yml` pins internal package
  `1.17.4-prerelease.20260825.3`; it runs manually through `workflow_dispatch` or automatically on
  the `yubikit-nativeshims-static` branch. Its package targets select a per-RID merged
