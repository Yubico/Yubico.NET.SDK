...
- **Discovery/connection ownership is atomic.** `DeviceConnectionRegistry` coordinates each interface `DeviceId` with connection leases and a nonblocking exclusive discovery lease. The lease belongs to the CONNECTION, not to a session: it is acquired before physical connect and released when that connection is disposed, so sessions come and go over a connection without touching it. **All interfaces (CCID, FIDO HID, OTP HID) are exclusive** — a second connection is refused immediately with `ConnectionInUseException` naming the interface. **CCID** is exclusive because the card holds one selected applet and a second SELECT would deselect the first holder's (measured, SW=0x6D00, see [Connection Ownership and Contention](../../docs/architecture/connection-ownership-and-contention.md)). This rule **is canonically motivated**: Rust `platform/pcsc.rs` `PcscConnection::open()` tries `ShareMode::Exclusive` first, falls back to shared, and will kill `scdaemon`/`yubikey-agent` to obtain exclusivity. The mechanism differs — canonical uses cross-process PC/SC share modes, we use an in-process lease — but the intent is the same. **OTP HID is exclusive** because one logical OTP frame spans multiple feature reports and separate protocol instances must not interleave them. **FIDO HID is exclusive** to ensure one native handle per physical interface. Unlike the CCID rule, neither OTP nor FIDO HID exclusivity is **canonical parity** — neither Rust yubikit nor Python yubikit enforces any in-process HID exclusivity (verified: `HidOtpConnection::new` and `FidoConnection::new` just open the path; neither produces two concurrent in-process handles for one interface). This SDK's HID exclusivity exists because we expose a public `ConnectAsync` a caller can invoke twice. macOS FIDO opens stay non-seizing (`kIOHIDOptionsTypeNone`); the in-process registry, not a platform seize flag, owns SDK admission. Management-over-HID fallback while CCID is held is supported: `YubiKeyConnectionExtensions.IsFallbackEligibleHeldError` treats an in-process `ConnectionInUseException` (any exclusive transport) exactly like a SmartCard PC/SC sharing violation, so the default order can try `HidFido`, see it refused, and then try `HidOtp`; an explicit `preferredConnection` never falls back. One level down, `ConnectionSessionGuard` allows one live `ApplicationSession` per connection and refuses a second with the session name. Discovery skips immediately while any member connection is active, and connections wait (cancellably) while discovery holds the interface across connect, Management exchange, and disposal. Discovery uses only the internal `IDiscoveryConnectionProvider` path; wrappers/custom `IYubiKey` implementations without it are skipped without calling public `ConnectAsync`. In-process only — cross-process contention is not covered. Idle coordinator entries are retained for the process lifetime to avoid unsafe eviction races and are bounded by unique interface IDs observed.
