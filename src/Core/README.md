...
- **Discovery never disturbs open sessions.** Per-interface connection leases and a nonblocking exclusive discovery lease make ownership atomic: a connection owns the interface before physical connect, discovery skips interfaces with a live connection, and a connect cannot cross a Management metadata read already in progress. CCID and OTP HID each admit one live connection at a time; FIDO HID remains shared. OTP is exclusive because one logical OTP frame spans multiple feature reports. Shared FIDO HID means a second connection is admitted, not that two handles can run concurrent CTAP conversations — on macOS input reports are not demultiplexed between handles, so drive CTAP over one FIDO connection at a time.
