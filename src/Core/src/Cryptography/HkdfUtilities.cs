...
internal static class HkdfUtilities
{
    ...
    {
        var key = salt.IsEmpty ? stackalloc byte[Sha256HashByteLength] : salt;
        return HMACSHA256.HashData(key, inputKeyMaterial);
    }
        ...
        int length)
    {
        ...
        {
            var inputSize = previousBlock.Length + contextInfo.Length + 1;
            ...
            var currentBlock = HMACSHA256.HashData(pseudoRandomKey.Span, input);
            ...
            var blockOffset = (index - 1) * Sha256HashByteLength;
            var bytesToCopy = Math.Min(Sha256HashByteLength, length - blockOffset);
            currentBlock
